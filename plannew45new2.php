...
<?php
// Retrieve message from URL query string
...
// Update availability_date for mold table
$updateMoldQuery = "UPDATE mold SET availability_date = DATE_FORMAT(DATE_ADD(CURRENT_DATE(), INTERVAL 7 HOUR), '%Y-%m-%d 07:00:00')";
$conn->query($updateMoldQuery);

// Update availability_date for cavity table
$updateCavityQuery = "UPDATE cavity SET availability_date = DATE_FORMAT(DATE_ADD(CURRENT_DATE(), INTERVAL 7 HOUR), '%Y-%m-%d 07:00:00')";
$conn->query($updateCavityQuery);
...
$query = "SELECT process.*, plannew1.end_date, plannew1.start_date 
          FROM process 
...
if ($tobeResult->num_rows > 0) {
    ...
    echo "</td>";
      ...
      // Checkbox for To Be First
      echo "<td><input type='checkbox' onchange='saveCheckboxData({$row['id']}, \"first_tobe\", this)'";
      if ($row['first_tobe'] == 1) {
          echo " checked";
      }
      echo "></td>";
    ...
    echo "<td><button onclick='goToAnotherPage1()'>Check highlight</button></td>"; // Added this line
...
echo "<script>
      ...
      echo "<script>
        function goToAnotherPage1() {
            // Specify the URL of the page you want to navigate to
            var newPageUrl = 'plannew45new22.php';
...
function searchByIcodeDescription() {
    ...
    for (let i = 1; i < rows.length; i++) { // Start from 1 to skip the header row
        const icodeCell = rows[i].getElementsByTagName('td')[3]; // Index 3 corresponds to the Icode column
        const descriptionCell = rows[i].getElementsByTagName('td')[11]; // Index 4 corresponds to the Description column
