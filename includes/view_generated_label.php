...
if ($brand_f !== '') {
    $where_parts[] = "gs.brand = ?";
    $params[] = $brand_f; $types .= 's';
}

if ($date_from !== '') {
    $where_parts[] = "DATE(gs.generated_at) >= ?";
...
// All brands for filter dropdown
...
// ── EXPORT CSV ────────────────────────────────────────────────────────────────
// (handled via a quick flag so we don't need a separate file)
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    // Re-run query without pagination for full export - just redirect with same filters
    // This is handled client-side by reloading with export param
}
...
.brand-name { font-family:'Bebas Neue',sans-serif; font-size:1.25rem; letter-spacing:0.08em; }
.topbar-right { display:flex; align-items:center; gap:12px; }
.topbar-date { font-size:0.75rem; font-weight:500; color:var(--muted); background:var(--surface2); border:1px solid var(--border); padding:7px 14px; border-radius:8px; }
...
.hero-strip::after { content:''; position:absolute; right:-80px; top:-80px; width:280px; height:280px; background:radial-gradient(circle,rgba(242,128,24,0.1) 0%,transparent 70%); pointer-events:none; }
...
.qs-card.active { border-color:var(--border-hot); background:linear-gradient(135deg,#1c0f00,var(--surface)); box-shadow:0 0 20px var(--orange-glow); }
...
.qs-info {}
.qs-val { font-family:'Bebas Neue',sans-serif; font-size:2rem; line-height:1; color:var(--white); }
...
.f-control {
    ...
    font-size:0.82rem; color:var(--white); outline:none; transition:border-color 0.18s;
}
.f-control:focus { border-color:var(--orange); box-shadow:0 0 0 3px var(--orange-soft); }
.f-control option { background:var(--surface2); }
.btn-apply {
...
.tbl-wrap thead th:hover { color:var(--orange); }
.tbl-wrap thead th i.sort-icon { margin-left:5px; font-size:0.55rem; opacity:0.5; }
...
.td-serial { font-weight:700; color:var(--white); font-size:0.88rem; letter-spacing:0.02em; }
.td-icode {
...
.td-date-main { color:var(--white); font-weight:600; font-size:0.78rem; }
...
.page-btn:hover { border-color:var(--border-hot); color:var(--orange); background:var(--orange-soft); }
...
@media(max-width:1000px){
    ...
    .tbl-wrap thead th:nth-child(5), .tbl-wrap tbody td:nth-child(5) { display:none; }
}
...
<header class="topbar">
    ...
    <div class="topbar-right">
        ...
        <a href="dashboard.php" class="btn-t btn-ghost"><i class="fas fa-tachometer-alt"></i> Dashboard</a>
...
<div class="page">
    ...
    <div class="filter-wrap">
        ...
        <div class="filter-tags">
            ...
            <?php endif; ?>
            <a href="?" class="f-tag" style="background:rgba(255,255,255,0.06);border-color:var(--border);color:var(--muted);text-decoration:none;"><i class="fas fa-times-circle"></i> Clear all</a>
        </div>
        <?php endif; ?>
    </div>
            ...
            <tbody>
                <?php foreach ($rows as $i => $row):
                    ...
                    function hlSearch($str, $q) {
                        ...
                        return preg_replace('/(' . preg_quote(htmlspecialchars($q), '/') . ')/i', '<span class="hl">$1</span>', htmlspecialchars($str ?? ''));
                    }
                ?>
                <tr>
                    <td class="td-num"><?php echo $row_num; ?></td>
                    <td>
                        <div class="td-serial"><?php echo hlSearch($row['serial_number'], $search); ?></div>
                    </td>
                    <td>
                        <span class="td-icode"><?php echo hlSearch($row['icode'], $search); ?></span>
                    </td>
    ...
    <?php
        $base_q = array_filter(['filter'=>$filter,'search'=>$search,'brand'=>$brand_f,'date_from'=>$date_from,'date_to'=>$date_to]);
    ?>
...
<script>
// Auto-submit search on Enter (already works via form, just UX)
