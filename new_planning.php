...
<body>
    ...
    <div class="label-container">
        <form method="GET" action="">
            ...
            <select name="press_name" id="press_name">
                ...
                $sql = "SELECT DISTINCT press_name FROM plannew1";
    ...
    <form method="GET" action="planning_new5.php">
        ...
        <select name="mold_id" id="mold_id">
            ...
            $sql = "SELECT DISTINCT mold_id FROM plannew1";
            $result = mysqli_query($conn, $sql);
            if ($result && mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
...
if (isset($_GET['press_name']) && !empty($_GET['press_name'])) {
    ...
    $erpSql = "SELECT erp, customer, MAX(end_date) as last_completion_date FROM plannew1 WHERE press_name = '$press_name' GROUP BY erp";
} else {
    // If the filter is not set or empty, retrieve all data
    $erpSql = "SELECT erp, customer, MAX(end_date) as last_completion_date FROM plannew1 GROUP BY erp";
}
...
if ($erpResult) {
    if (mysqli_num_rows($erpResult) > 0) {
        ...
        while ($erpRow = mysqli_fetch_assoc($erpResult)) {
            ...
            if (isset($_GET['press_name']) && !empty($_GET['press_name'])) {
                $sql = "SELECT * FROM plannew1 WHERE erp = '$erp' AND press_name = '$press_name'";
            } else {
                $sql = "SELECT * FROM plannew1 WHERE erp = '$erp'";
            }
            $result = mysqli_query($conn, $sql);
