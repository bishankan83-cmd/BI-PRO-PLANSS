...
<body>
    ...
    if (isset($_GET['mold_id']) && !empty($_GET['mold_id'])) {
        ...
        $erpSql = "SELECT erp, customer, MAX(end_date) as last_completion_date FROM new_plan_data WHERE mold_id = '$mold_id' GROUP BY erp";
    } else {
    ...
    if ($erpResult) {
        if (mysqli_num_rows($erpResult) > 0) {
            ...
            while ($erpRow = mysqli_fetch_assoc($erpResult)) {
                ...
                if (isset($_GET['mold_id']) && !empty($_GET['mold_id'])) {
                    $sql = "SELECT * FROM new_plan_data WHERE erp = '$erp' AND mold_id = '$mold_id'";
                } else {
