...
try {
    ...
    $sql = "
    DELETE t1 FROM another_table_name t1
    INNER JOIN another_table_name t2 
    WHERE 
        t1.id > t2.id AND 
...
document.getElementById("redirectButtonn").onclick = function() {
    ...
    window.location.href = "barcode_enter.php";
};
</script>
...
$query = "SELECT * FROM another_table_name ORDER BY CAST(batch AS UNSIGNED) ASC";
...
if(isset($_POST['submit'])) {
    $update_query = "UPDATE another_table_name SET quality_approved=?, expire_date=?, staff_name=?, sg_value=?, hardness=?, mh=?, ml=?, t10=?, t90=?, t52=?, rebound=? WHERE id=?";
