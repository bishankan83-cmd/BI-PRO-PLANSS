...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['action']) && $_POST['action'] === 'yes') {
            ...
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM saved_tires WHERE serialNumber = ?");
            $stmt->execute([$serialNumber]);
            $existingCount = $stmt->fetchColumn();

            if ($existingCount > 0) {
                // Display message if serial number exists
                echo "<p style='color: red; font-size: 20px;'>This serial number already exists in the database.</p>";
                echo "<a href='index.php' style='font-size: 18px;'>Go back and try again.</a>";
                exit;
            }
            ...
            // Insert into the saved_tires table (for historical records)
            $stmt = $pdo->prepare("INSERT INTO saved_tires (serialNumber, tireCode, brand, tireWeight, pressNumber) VALUES (?, ?, ?, ?, ?)");
...
<head>
    ...
    <script>
        function printQRCode() {
            window.location.href = 'process12.php'; // Replace 'another_page.php' with the page you want to redirect to
        }
    </script>
</head>
<body>
