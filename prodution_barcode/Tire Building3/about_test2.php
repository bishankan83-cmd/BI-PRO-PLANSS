...
if (!empty($serialNumber)) {
    ...
    // Get tire code information for display
    $stmt = $conn->prepare("SELECT tire_code FROM qr_scanned_data WHERE serial_number = ?");
    $stmt->bind_param("s", $serialNumber);
    $stmt->execute();
    $tireResult = $stmt->get_result();
    $tireCode = ($tireResult->num_rows > 0) ? $tireResult->fetch_assoc()['tire_code'] : 'N/A';
    $stmt->close();
    
    // Check if all three conditions are met
    if ($row1['count'] > 0 && $row2['count'] > 0 && $row3['count'] > 0) {
        $buttonEnabled = true;
        $message = "<div class='success-message'>Serial Number validation successful! All components have been scanned.</div>";
    } else {
        $scannedCount = 0;
        $missingItems = [];
        ...
        $message = "<div class='error-message'>
                      <p>Serial Number validation failed. $scannedCount/3 components scanned.</p>
                      <p>Missing components: " . implode(", ", $missingItems) . "</p>
                    </div>";
    }
}
$conn->close();
    ...
    <div class="main-container">
        ...
        <form method="GET" class="search-form">
            <input type="text" class="search-input" id="serialNumber" name="serialNumber" 
                   placeholder="Enter or scan serial number" value="<?php echo htmlspecialchars($serialNumber); ?>" required>
        ...
        <?php if (!empty($serialNumber)): ?>
            ...
            <div class="result-section">
                <div class="info-item">
                    ...
                    <div class="info-value"><?php echo htmlspecialchars($serialNumber); ?></div>
                </div>
                
                <?php if (isset($tireCode)): ?>
                <div class="info-item">
                    <span class="info-label">Tire Code</span>
                    <div class="info-value"><?php echo htmlspecialchars($tireCode); ?></div>
                </div>
                <?php endif; ?>
                ...
                <div class="progress-container">
                    ...
                    <?php 
                    $progressPercentage = isset($scannedCount) ? ($scannedCount / 3) * 100 : 0;
                    ?>
                    <div class="progress-bar">
                        <div class="progress-fill" style="width: <?php echo $progressPercentage; ?>%">
                            <?php echo isset($scannedCount) ? "$scannedCount/3" : "0/3"; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="scan-status">
                    <div class="info-item">
                        ...
                        <div class="info-value">
                            <div><i class="fas <?php echo $row1['count'] > 0 ? 'fa-check-circle text-success' : 'fa-times-circle text-danger'; ?>"></i> Base Component</div>
                            <div><i class="fas <?php echo $row2['count'] > 0 ? 'fa-check-circle text-success' : 'fa-times-circle text-danger'; ?>"></i> Cusion Component</div>
                            <div><i class="fas <?php echo $row3['count'] > 0 ? 'fa-check-circle text-success' : 'fa-times-circle text-danger'; ?>"></i> Thread Component</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <p style="text-align: center; margin-top: 10px; color: #721c24;">
                <i class="fas fa-exclamation-circle"></i> Please complete all component scans to proceed.
            </p>
            <?php endif; ?>
        <?php endif; ?>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            input.addEventListener('keydown', function(e) {
                ...
                if (e.key === 'Enter' && timeDiff < 100 && input.value.length > 5) {
                    e.preventDefault();
