...
$sql = "SELECT s.id, s.icode, s.tires_per_mold, s.mold_id, s.cavity_id, t.time_taken, p.erp, m.availability_date AS mold_availability, c.availability_date AS cavity_availability, s.start_date AS process_start_date
        ...
        INNER JOIN tobeplan_plan p ON s.icode = p.icode
...
if (mysqli_num_rows($result) > 0) {
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    // Calculate the start date based on the latest end date of the previous tire type or the current process_plan start date,
    ...
    // Check for holidays between start date and end date
    $holidayQuery = "SELECT COUNT(*) AS num_holidays FROM holidays WHERE holiday_date BETWEEN '$start_date' AND '$end_date'";
    $holidayResult = mysqli_query($conn, $holidayQuery);
    if ($holidayResult) {
      $holidayRow = mysqli_fetch_assoc($holidayResult);
      $num_holidays = $holidayRow['num_holidays'];
      if ($num_holidays > 0) {
        // Add number of holidays to end date
        $end_date = date("Y-m-d H:i:s", strtotime("$end_date + $num_holidays days"));
      }
    } else {
      die("Query failed: " . mysqli_error($conn));
    }
    ...
    $sql = "INSERT INTO plannew1 (id, icode, mold_id, cavity_id, start_date, end_date, erp, tires_per_mold, Customer)
            VALUES ($id, '$icode', '$mold', '$cavity', '$start_date', '$end_date', '$erp_number', '$tobe', '$customer_name')";
