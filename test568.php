 ...
 WHERE loan_number = ?";
        ...
        if ($row = $result->fetch_assoc()) {
           ...
           if (!$check_stmt) {
            ...
            $check_stmt->bind_param("s", $selected_po_number);
            $check_stmt->execute();
            $check_result = $check_stmt->get_result();
            
            if ($check_result->num_rows > 0) {
                throw new Exception("This PO number already exists in purchase_orders2!");
            }
            ...
            $insert_stmt->bind_param("sssssssiis", 
                $selected_po_number,
                $row['po_date'],
                $row['expected_deliver_inhouse_date'],
                $row['supplier_code'],
                $row['supplier_name'],
                $row['rm_code'],
                $row['descriptions'],
            ...
            if (!$insert_stmt->execute()) {
                throw new Exception("Error inserting into purchase_orders2: " . $insert_stmt->error);
            }
            ...
            echo "<div style='color: green; margin: 10px 0;'>
                    Data successfully inserted into purchase_orders2 and stock updated!</div>";
            ...
            throw new Exception("No record found for PO Number: " . htmlspecialchars($selected_po_number));
        }
...
<form method="post" action="">
    <label for="po_number">Choose PO Number:</label>
