...
try {
    ...
    // SQL query to delete all data from the account table
    $sql = "DELETE FROM account"; // or "DELETE FROM account WHERE Icode = 'some_value'";

    // Prepare and execute the query
    $stmt = $pdo->prepare($sql);
    $stmt->execute();

    echo $stmt->rowCount() . " records deleted successfully."; // Number of deleted records
} catch (PDOException $e) {
    echo "Error: " . htmlspecialchars($e->getMessage()); // Error handling
}
...
try {
    ...
    // Insert each result into the account table
    $insert_sql = "INSERT INTO account (Icode, tire_description, CavityName, Date, mold_size, mold_id) 
                   VALUES (:Icode, :tire_description, :CavityName, :Date, :mold_size, :mold_id)";
    $insert_stmt = $pdo->prepare($insert_sql);

    foreach ($results as $row) {
        $insert_stmt->execute([
            ':Icode' => $row['Icode'],
            ':tire_description' => $row['tire_description'] ?? null,
            ':CavityName' => $row['CavityName'],
            ':Date' => $row['Date'],
            ':mold_size' => $row['mold_size'] ?? null,
            ':mold_id' => $row['mold_id'] ?? null
        ]);
    }

    echo "Data has been successfully inserted into the 'account' table.";
