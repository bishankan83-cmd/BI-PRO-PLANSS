...
if (isset($_POST['insert_data']) && $selected_po_number) {
    ...
    try {
        ...
        if ($row = $result->fetch_assoc()) {
            // Check if PO number already exists in loan_inward_details_settle
            $check_sql = "SELECT loan_number FROM loan_inward_details_settle WHERE loan_number = ?";
            ...
            if ($check_result->num_rows > 0) {
                throw new Exception("This Loan number already exists in loan_inward_details_settle!");
            }

            // Insert into loan_inward_details_settle
            $insert_sql = "INSERT INTO loan_inward_details_settle 
            ...
            if (!$insert_stmt->execute()) {
                throw new Exception("Error inserting into loan_inward_details_settle: " . $insert_stmt->error);
            }
            ...
            echo "<div style='color: green; margin: 10px 0;'>
                    Data successfully inserted into loan_inward_details_settle and stock updated!</div>";
...
<form method="post" action="">
    ...
    <select id="po_number" name="po_number" required>
        ...
        <?php
        $result = $conn->query("SELECT loan_number FROM loan_inward_details2 ORDER BY loan_number");
...
if ($selected_po_number) {
    ...
    if ($stmt) {
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            echo "</table>";
            echo "<button type='submit' name='insert_data'>Insert Into loan_inward_details_settle and Update Stock</button>";
            echo "</form>";
        } else {
