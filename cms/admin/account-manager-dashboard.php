...
$managerConfirmDiscSuccessOrders = 0;
...
if ($isAccountManager && !empty($userIds)) {
    ...
    $managerConfirmDiscSuccessOrders = qCount($con, "SELECT COUNT(*) as count FROM tire_orders WHERE status='manager_confirm_disc_success' AND customer_id IN ($userIdsStr)");
} else {
    ...
    $managerConfirmDiscSuccessOrders = pCount($con, "SELECT COUNT(*) as count FROM tire_orders WHERE status='manager_confirm_disc_success'");
...
$notificationCount = (int)($customerConfirmedOrders > 0)
                   + (int)($cusPiConfirmOrders > 0)
                   + (int)($requestReviseOrders > 0)
                   + (int)($managerConfirmDiscSuccessOrders > 0);
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
        ...
        .alert-banner.alert-info    { background:var(--info);    box-shadow:0 10px 35px rgba(52,152,219,0.35); }
        .alert-banner.second { top:152px; }
        .alert-banner.third  { top:224px; }
        .alert-banner.fourth { top:296px; }
        ...
        .alert-info    .alert-count { color:var(--info);    }
        ...
        body.notify-4 .main-scroll { margin-top:288px; }
        ...
        .status-disc-success  { background:var(--info-light);    color:var(--info); border:1px solid rgba(52,152,219,0.25); }
...
<body class="<?php
    ...
    elseif ($notificationCount >= 4) echo 'notify-4';
?>">
...
<?php
// Calculate banner positions dynamically
$bannerPos = 0;
$posClass  = ['', 'second', 'third', 'fourth'];

$b1 = $customerConfirmedOrders > 0;
$b2 = $cusPiConfirmOrders > 0;
$b3 = $requestReviseOrders > 0;
$b4 = $managerConfirmDiscSuccessOrders > 0;
?>
...
<?php if ($b2):
    $p2 = (int)$b1;
    $cls2 = $p2 > 0 ? $posClass[$p2] : '';
?>
<div class="alert-banner alert-success fade-in <?php echo $cls2; ?>" id="notif2" onclick="window.location.href='tire-orders-marketing.php?status=cus_pi_confirm'">
...
<?php if ($b3):
    $p3 = (int)$b1 + (int)$b2;
    $cls3 = $p3 > 0 ? $posClass[$p3] : '';
?>
<div class="alert-banner alert-warning fade-in <?php echo $cls3; ?>" id="notif3" onclick="window.location.href='tire-orders-revise.php?request_status=request_revise'">
...
<?php if ($b4):
    $p4 = (int)$b1 + (int)$b2 + (int)$b3;
    $cls4 = $p4 > 0 ? $posClass[$p4] : '';
?>
<div class="alert-banner alert-info fade-in <?php echo $cls4; ?>" id="notif4" onclick="window.location.href='tire-orders.php?status=manager_confirm_disc_success'">
    <div class="alert-icon"><i class="fas fa-tag"></i></div>
    <div class="alert-content">
        <div class="alert-title">💰 Discount Approved — Action Required</div>
        <div class="alert-msg">Manager-confirmed discount order<?php echo $managerConfirmDiscSuccessOrders > 1 ? 's are' : ' is'; ?> awaiting next step.</div>
    </div>
    <span class="alert-count"><?php echo $managerConfirmDiscSuccessOrders; ?></span>
    <a href="tire-orders.php?status=manager_confirm_disc_success" class="alert-action" onclick="event.stopPropagation()"><i class="fas fa-eye"></i> View Orders</a>
    <div class="alert-close" onclick="event.stopPropagation(); closeAlert('notif4')"><i class="fas fa-times"></i></div>
</div>
<?php endif; ?>
...
<div class="page-wrapper">
    ...
    <div class="container">
        ...
        <div class="page-header reveal">
            ...
            <div class="page-header-badge"><i class="fas fa-shield-alt"></i> Admin Access</div>
        </div>
        ...
        <div class="stats-grid reveal">
            ...
            <div class="stat-card info" onclick="location.href='tire-orders.php?status=manager_confirm_disc_success'">
                <div class="stat-header">
                    <div class="stat-icon" style="background:var(--info-light);color:var(--info);"><i class="fas fa-tag"></i></div>
                    <div class="stat-trend" style="background:var(--info-light);color:var(--info);"><i class="fas fa-check"></i> Disc.</div>
                </div>
                <div class="stat-value" style="color:var(--info);"><?php echo $managerConfirmDiscSuccessOrders; ?></div>
                <div class="stat-label">Discount Approved</div>
                <div class="stat-description">Manager-confirmed discounts</div>
                <a href="tire-orders.php?status=manager_confirm_disc_success" class="stat-link" style="color:var(--info);">View <i class="fas fa-arrow-right"></i></a>
            </div>
        </div>
        ...
        <div class="quick-actions reveal">
            ...
            <a href="tire-orders.php?status=manager_confirm_disc_success" class="quick-action-card">
                <div class="quick-action-icon"><i class="fas fa-tag"></i></div>
                <div class="quick-action-title">Disc. Approved</div>
                <div class="quick-action-subtitle"><?php echo $managerConfirmDiscSuccessOrders; ?> action needed</div>
            </a>
                ...
                <div class="list-container">
                    ...
                    <a href="order-details.php?oid=<?php echo $order['order_id']; ?>" class="list-item">
                        ...
                        <div class="status-badge <?php
                            ...
                            elseif ($order['status'] == 'manager_confirm_disc_success') echo 'status-disc-success';
                            else                                                         echo 'status-pending';
                        ?>">
                            <?php
                            if ($order['status'] == 'cus_confirmed')                    echo 'Cus. Confirmed';
                            elseif ($order['status'] == 'cus_pi_confirm')               echo 'PI Confirmed';
                            elseif ($order['status'] == 'manager_confirm_disc_success') echo 'Disc. Approved';
...
function reStackAlerts() {
    ...
    alerts.forEach((a, i) => {
        a.classList.remove('second', 'third', 'fourth');
        if (classes[i]) a.classList.add(classes[i]);
    });
}
</script>
</body>
