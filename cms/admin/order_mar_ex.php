...
if (strlen($_SESSION['aid']) == 0) {
    ...
    // === STATUS FILTER LOGIC ===
    $currentStatus = $_GET['status'] ?? 'cus_confirmed';
    $validStatuses = ['cus_confirmed', 'cus_pi_confirm', 'confirm_marketing', 'Manager_confirm_discount']; // ✅ Added new status
    $statusFilter = '';
    ...
    if (in_array($currentStatus, $validStatuses)) {
        $statusFilter = " WHERE o.status = '" . mysqli_real_escape_string($con, $currentStatus) . "'";
        ...
        } elseif ($currentStatus === 'Manager_confirm_discount') { // ✅ New block
            $pageTitle = 'Manager Confirmed Discount Orders';
            $pageSubtitle = 'Orders where discount has been confirmed by manager';
            $statusLabel = 'Manager Confirm Discount';
        }
    }
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .filter-tab.active { background: var(--gradient-1); color: white; }
        ...
        /* ✅ New badge style */
        .status-Manager_confirm_discount { background: #d4edda; color: #155724; border: 1px solid #a8d5b5; }
...
<body>
    ...
    <div class="main-wrapper">
        <a href="mar_ex.php" class="btn btn-back">
        ...
        <div class="page-header">
            ...
            <div class="filter-tabs">
                <a href="?status=cus_confirmed" class="filter-tab <?php echo $currentStatus === 'cus_confirmed' ? 'active' : ''; ?>">
                    <i class="fas fa-check-circle"></i> Customer Confirmed
                </a>
                <a href="?status=cus_pi_confirm" class="filter-tab <?php echo $currentStatus === 'cus_pi_confirm' ? 'active' : ''; ?>">
                    <i class="fas fa-file-invoice"></i> Waiting for PI
                </a>
                <!-- ✅ New Tab Added -->
                <a href="?status=Manager_confirm_discount" class="filter-tab <?php echo $currentStatus === 'Manager_confirm_discount' ? 'active' : ''; ?>">
                    <i class="fas fa-tags"></i> Manager Confirm Discount
                </a>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="info-badge">
                    Total:
                    <?php
                    $countQuery = "SELECT COUNT(*) as t FROM tire_orders o $statusFilter";
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <!-- ✅ Status badge column -->
                                    <td>
                                        <span class="status-badge status-<?php echo htmlentities($row['status']); ?>">
                                            <?php echo htmlentities($row['status']); ?>
                                        </span>
                                    </td>
                                    <td>
                                        <a href="tire-order_mar_ex.php?id=<?php echo $row['order_id']; ?>" class="btn btn-primary">
                                ...
                                <tr>
                                    <td colspan="9">
                                        <div class="no-data">
                                            ...
                                            <p>There are no orders with <strong><?php echo htmlentities($statusLabel); ?></strong> status.</p>
                                        </div>
                                    </td>
                                </tr>
                            <?php } ?>
