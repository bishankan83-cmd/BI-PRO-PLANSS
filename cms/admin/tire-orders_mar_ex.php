...
if (strlen($_SESSION['aid']) == 0) {
    ...
    // === STATUS FILTER LOGIC ===
    $currentStatus = $_GET['status'] ?? ''; // Default to empty = show all
    $validStatuses = ['cus_confirmed', 'cus_pi_confirm', 'confirm_marketing'];
    $statusFilter = '';
    $pageTitle = 'All Orders';
    $pageSubtitle = 'Showing all orders across all statuses';
    $statusLabel = 'All';
    ...
    if (!empty($currentStatus) && in_array($currentStatus, $validStatuses)) {
        ...
        if ($currentStatus === 'cus_confirmed') {
        ...
        } elseif ($currentStatus === 'cus_pi_confirm') {
            ...
            $pageTitle = 'Marketing Confirmed Orders';
            $pageSubtitle = 'Orders confirmed by marketing';
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .status-confirm_marketing{ background: #d4edda; color: #155724; border: 1px solid #b8dfc4; }
    ...
    <div class="main-wrapper">
        ...
        <div class="page-header">
            ...
            <div class="filter-tabs">
                <a href="?" class="filter-tab <?php echo empty($currentStatus) ? 'active' : ''; ?>">
                            ...
                            if ($query && mysqli_num_rows($query) > 0) {
                                while ($row = mysqli_fetch_array($query)) {
                                    ...
                                    $statusMap = [
                                        ...
                                        'cus_pi_confirm'    => 'Waiting for PI',
                                        'confirm_marketing' => 'Marketing Confirmed',
                                    ];
                                    $statusKey   = $row['status'] ?? '';
                                    $statusText  = $statusMap[$statusKey] ?? ucwords(str_replace('_', ' ', $statusKey));
                                    $badgeClass  = 'status-' . htmlentities($statusKey);
                            ?>
                                <tr>
                                    <td><?php echo $cnt++; ?></td>
