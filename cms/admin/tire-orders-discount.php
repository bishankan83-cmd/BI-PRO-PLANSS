...
if (strlen($_SESSION['aid']) == 0) {
    ...
    // === STATUS FILTER LOGIC ===
    $currentStatus = $_GET['status'] ?? 'manager_confirm_disc_success'; // Default to manager_confirm_disc_success
    $validStatuses = ['confirm_marketing', 'manager_confirm_disc_success'];
    $statusFilter = '';
    $pageTitle = 'Manager Confirmed Discount Success';
    $pageSubtitle = 'Orders where manager has confirmed discount successfully';
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Special green tab for manager_confirm_disc_success */
        .filter-tab.tab-success {
            background: #e8f8f0; color: #1a7a45;
        }
        .filter-tab.tab-success.active {
            background: linear-gradient(135deg, #27ae60 0%, #1a7a45 100%); color: white;
        }
        .filter-tab.tab-success:hover:not(.active) { background: #d0f0e0; }
        ...
        .info-badge.badge-success { background: rgba(39, 174, 96, 0.12); color: #1a7a45; }
        ...
        .status-manager_confirm_disc_success { background: #d4edda; color: #155724; border: 1px solid #c3e6cb; }
    ...
    <div class="main-wrapper">
        ...
        <div class="card">
            <div class="card-header">
                <h2 class="card-title">
                    <?php if ($currentStatus === 'manager_confirm_disc_success'): ?>
                        <i class="fas fa-tag" style="color:#27ae60;"></i>
                    <?php else: ?>
                        <i class="fas fa-clipboard-check"></i>
                    <?php endif; ?>
                    <?php echo htmlentities($pageTitle); ?>
                </h2>
                <div class="info-badge <?php echo $currentStatus === 'manager_confirm_disc_success' ? 'badge-success' : ''; ?>">
                    Total: 
                    <?php
                            ...
                            if (!empty($allowedCustomerIds)) {
                                ...
                                if (mysqli_num_rows($query) > 0) {
                                    while ($row = mysqli_fetch_array($query)) {
                                        $isDiscSuccess = $row['status'] === 'manager_confirm_disc_success';
                            ?>
                                    <tr>
                                        <td><?php echo $cnt++; ?></td>
                                        ...
                                        <td>
                                            <?php if ($isDiscSuccess): ?>
                                                <span class="status-badge status-manager_confirm_disc_success" style="display:inline-flex;align-items:center;gap:0.4rem;margin-bottom:0.4rem;">
                                                    <i class="fas fa-check-circle"></i> Disc. Confirmed
                                                </span><br>
                                            <?php endif; ?>
                                            <a href="gate_order.php?id=<?php echo $row['order_id']; ?>" class="btn btn-primary">
