...
if (strlen($_SESSION['aid']) == 0) {
    ...
    // Handle acm_comment update — redirects to view-tire-orders-pi-mar.php
    if (isset($_POST['update_acm_comment'])) {
        $acmComment = mysqli_real_escape_string($con, $_POST['acm_comment']);

        $updateAcm = mysqli_query($con, "
            UPDATE tire_orders 
            SET acm_comment = '$acmComment' 
            WHERE order_id = '$orderId'
        ");

        if ($updateAcm) {
            $_SESSION['success_msg'] = "ACM comment updated successfully!";
            header('Location: view-tire-orders-pi-mar.php?id=' . urlencode($orderId));
            exit();
        } else {
            $errorMsg = "Failed to update ACM comment.";
        }
    }

    // Fetch order details — no customer restriction
...
<body>
    ...
    <div class="main-wrapper">
        <!-- Back Button -->
        <a href="tire-orders-marketing-share-planning.php" class="btn btn-back">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="info-grid">
                    ...
                    <!-- ACM Comment Display -->
                    <div class="info-item" style="grid-column: 1 / -1;">
                        <div class="info-label">ACM Comment</div>
                        <div class="info-value" style="font-weight: 400;">
                            ...
                            <?php else: ?>
                                <div class="acm-display empty">No ACM comment has been added yet.</div>
                            <?php endif; ?>
                        </div>
                    </div>
        ...
        <!-- ACM Comment Edit Card -->
        <div class="card">
            <div class="card-header">
                <h2 class="card-title">
                    <i class="fas fa-comment-dots"></i> Edit ACM Comment
                </h2>
            </div>
            <div class="card-body">
                <form method="POST" action="">
                    <div class="form-group">
                        <label class="form-label" for="acm_comment">ACM Comment</label>
                        <textarea
                            id="acm_comment"
                            name="acm_comment"
                            class="form-control"
                            rows="6"
                            placeholder="Enter ACM comment here..."
                        ><?php echo htmlentities($orderData['acm_comment'] ?? ''); ?></textarea>
                    </div>
                    <button type="submit" name="update_acm_comment" class="btn btn-primary">
                        <i class="fas fa-save"></i> Save ACM Comment
                    </button>
                    <p class="redirect-note">
                        <i class="fas fa-share-square"></i>
                        After saving, you will be redirected to the PI/MAR order view page.
                    </p>
                </form>
            </div>
        </div>
