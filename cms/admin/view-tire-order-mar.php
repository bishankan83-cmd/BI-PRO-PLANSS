...
if (strlen($_SESSION['aid']) == 0) {
    ...
    if (isset($_POST['update_status'])) {
        ...
        if ($updateQuery) {
            ...
            if ($newStatus === 'Share_planning' || $newStatus === 'cus_pi_confirm') {
                ...
                header('Location: sent_mail_acc.php?id=' . urlencode($orderId));
            }
            exit();
        } else {
    ...
    if (isset($_POST['post_comment'])) {
        $newComment = mysqli_real_escape_string($con, trim($_POST['acm_comment']));

        $commentQuery = mysqli_query($con, "
            UPDATE tire_orders 
            SET acm_comment = '$newComment'
            WHERE order_id = '$orderId'
        ");

        if ($commentQuery) {
            $_SESSION['success_msg'] = "Comment saved successfully!";
        } else {
            $errorMsg = "Failed to save comment.";
        }
        header('Location: ' . $_SERVER['PHP_SELF'] . '?id=' . urlencode($orderId));
        exit();
    }
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-success:disabled { background: #cccccc; cursor: not-allowed; transform: none; }
        .btn-info { background: var(--gradient-3); color: white; box-shadow: var(--shadow); }
        .btn-info:hover { transform: translateY(-2px); box-shadow: var(--shadow-lg); }
        .btn-comment { background: var(--gradient-1); color: white; box-shadow: var(--shadow); }
        .btn-comment:hover { transform: translateY(-2px); box-shadow: var(--shadow-lg); }
        ...
        .table th, .table td { padding: 0.75rem; text-align: left; border-bottom: 1px solid var(--border-gray); font-size: 0.9rem; }
        .table th { font-weight: 600; color: var(--text-gray); text-transform: uppercase; font-size: 0.8rem; background: var(--bg-light); }
        ...
        .calculated-value { font-weight: 700; color: var(--primary-orange); }
        ...
        .comment-display {
            ...
            line-height: 1.7; color: var(--dark-gray); font-size: 0.95rem; white-space: pre-wrap;
        }
        .comment-empty { color: var(--text-gray); font-style: italic; }
        .comment-meta {
            display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.75rem;
            font-size: 0.85rem; color: var(--text-gray); font-weight: 600;
        }
        .comment-meta i { color: var(--primary-orange); }
        ...
        .divider { border: none; border-top: 2px dashed var(--border-gray); margin: 1.5rem 0; }
...
<div class="main-wrapper">
                    ...
                    <tbody>
                        ...
                        <tr>
                                    ...
                                    <ul class="multi-value-list">
                                        <?php foreach ($icodes as $icode):
                                            if (!empty($icode)) {
                                                ...
                                                $tireQuery = mysqli_query($con, "SELECT description FROM tire_details WHERE icode = '$icode_clean'");
                                                $tire = mysqli_fetch_array($tireQuery);
                                        ?>
                                                <li class="multi-value-item"><?php echo $tire ? htmlentities($tire['description']) : 'N/A'; ?></li>
                                        <?php } endforeach; ?>
                                    </ul>
                                <?php else:
                                    ...
                                    $tireQuery = mysqli_query($con, "SELECT description FROM tire_details WHERE icode = '$icode_clean'");
                                    $tire = mysqli_fetch_array($tireQuery);
                                    echo $tire ? htmlentities($tire['description']) : 'N/A';
                                endif; ?>
                            </td>
                                ...
                                <?php if (count($icodes) > 1): ?>
                                    <ul class="multi-value-list">
                                        <?php foreach ($icodes as $icode):
                                            if (!empty($icode)) {
                                                $icode_clean = mysqli_real_escape_string($con, $icode);
                                                $tireQuery = mysqli_query($con, "SELECT fweight FROM tire_details WHERE icode = '$icode_clean'");
                                                $tire = mysqli_fetch_array($tireQuery);
                                        ?>
                                                <li class="multi-value-item"><?php echo $tire ? htmlentities($tire['fweight']) : 'N/A'; ?></li>
                                        <?php } endforeach; ?>
                                    </ul>
                                <?php else:
                                    $icode_clean = mysqli_real_escape_string($con, $icodes[0]);
                                    $tireQuery = mysqli_query($con, "SELECT fweight FROM tire_details WHERE icode = '$icode_clean'");
                                    $tire = mysqli_fetch_array($tireQuery);
                                    echo $tire ? htmlentities($tire['fweight']) : 'N/A';
                                endif; ?>
                            </td>
                                ...
                                <?php if (count($icodes) > 1): ?>
                                    <ul class="multi-value-list">
                                        <?php foreach ($icodes as $icode):
                                            if (!empty($icode)) {
                                                $icode_clean = mysqli_real_escape_string($con, $icode);
                                                $tireQuery = mysqli_query($con, "SELECT cbm FROM tire_details WHERE icode = '$icode_clean'");
                                                $tire = mysqli_fetch_array($tireQuery);
                                        ?>
                                                <li class="multi-value-item"><?php echo $tire ? htmlentities($tire['cbm']) : 'N/A'; ?></li>
                                        <?php } endforeach; ?>
                                    </ul>
                                <?php else:
                                    $icode_clean = mysqli_real_escape_string($con, $icodes[0]);
                                    $tireQuery = mysqli_query($con, "SELECT cbm FROM tire_details WHERE icode = '$icode_clean'");
                                    $tire = mysqli_fetch_array($tireQuery);
                                    echo $tire ? htmlentities($tire['cbm']) : 'N/A';
                                endif; ?>
                            </td>
                            ...
                            <td>
                                <?php if (count($icodes) > 1): ?>
                                    <ul class="multi-value-list">
                                        <?php foreach ($icodes as $index => $icode):
                                            if (!empty($icode)) {
                                                $icode_clean = mysqli_real_escape_string($con, $icode);
                                                $tireQuery = mysqli_query($con, "SELECT fweight FROM tire_details WHERE icode = '$icode_clean'");
                                                $tire = mysqli_fetch_array($tireQuery);
                                                $fweight = $tire ? floatval($tire['fweight']) : 0;
                                                $qty = isset($quantities[$index]) ? floatval($quantities[$index]) : 0;
                                                $totalTonnage = $fweight * $qty;
                                        ?>
                                                <li class="multi-value-item">
                                                    <span class="calculated-value"><?php echo number_format($totalTonnage, 2); ?> kgs</span>
                                                </li>
                                        <?php } endforeach; ?>
                                    </ul>
                                <?php else:
                                    ...
                                    <ul class="multi-value-list">
                                        <?php foreach ($icodes as $index => $icode):
                                            if (!empty($icode)) {
                                                ...
                                                $tireQuery = mysqli_query($con, "SELECT cbm FROM tire_details WHERE icode = '$icode_clean'");
                                                $tire = mysqli_fetch_array($tireQuery);
                                                $cbm = $tire ? floatval($tire['cbm']) : 0;
                                                $qty = isset($quantities[$index]) ? floatval($quantities[$index]) : 0;
                                                $totalVolume = $cbm * $qty;
                                        ?>
                                                <li class="multi-value-item">
                                                    <span class="calculated-value"><?php echo number_format($totalVolume, 2); ?> cbm</span>
                                                </li>
                                        <?php } endforeach; ?>
                                    </ul>
                                <?php else:
    ...
    <div class="card">
        <div class="card-header">
            <h2 class="card-title"><i class="fas fa-comment-dots"></i> Order Comment</h2>
        </div>
        <div class="card-body">
            ...
            <?php if (!empty($orderData['acm_comment'])): ?>
                <div class="comment-meta">
                    <i class="fas fa-comment-check"></i> Current Comment
                </div>
                <div class="comment-display">
                    <?php echo nl2br(htmlentities($orderData['acm_comment'])); ?>
                </div>
            <?php else: ?>
                <div class="comment-display comment-empty">
                    <i class="fas fa-comment-slash" style="margin-right:0.4rem;"></i> No comment posted yet.
                </div>
            <?php endif; ?>
            ...
            <form method="POST" action="">
                <div class="form-group">
                    <label class="form-label" for="acm_comment">
                        <?php echo !empty($orderData['acm_comment']) ? 'Edit Comment' : 'Post a Comment'; ?>
                    </label>
                    <textarea
                        name="acm_comment"
                        id="acm_comment"
                        ...
                        rows="5"
                    ><?php echo htmlentities($orderData['acm_comment'] ?? ''); ?></textarea>
                </div>
                <button type="submit" name="post_comment" class="btn btn-comment">
                    <i class="fas fa-paper-plane"></i>
                    <?php echo !empty($orderData['acm_comment']) ? 'Update Comment' : 'Post Comment'; ?>
                </button>
            </form>
            ...
            <form method="POST" action="">
                <div class="form-group">
                    ...
                    <select name="status" id="status" class="form-control" required>
                        ...
                        <option value="confirm_marketing" <?php echo ($orderData['status'] == 'confirm_marketing') ? 'selected' : ''; ?>>Confirm Marketing</option>
                    </select>
                </div>
