...
if (!$user_id) {
    ...
    <body>
        <div class="error-box">
            ...
            <p>You must be logged in to access the digital signature page.</p>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && 
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'save_signature') {
        ...
        // Update signature
        $sql = "UPDATE users SET digital_signature = ?, signature_date = NOW() WHERE id = ?";
        ...
        $stmt->bind_param("si", $signature_data, $user_id);
        
        if ($stmt->execute()) {
            // Verify the save
            $verify_stmt = $conn->prepare("SELECT LENGTH(digital_signature) as sig_length, signature_date FROM users WHERE id = ?");
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'clear_signature') {
        $stmt = $conn->prepare("UPDATE users SET digital_signature = NULL, signature_date = NULL WHERE id = ?");
...
// Fetch user details
$stmt = $conn->prepare("SELECT id, fullName, userEmail, digital_signature, signature_date FROM users WHERE id = ?");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$result = $stmt->get_result();
...
if (!$user) {
    die("User not found in database. Please contact administrator.");
}
?>
...
<head>
    ...
    <title>Digital Signature - <?php echo htmlspecialchars($user['fullName']); ?></title>
...
<body>
    ...
    <div class="container">
        ...
        <div class="header">
            <h1><i class="fas fa-signature"></i> Digital Signature</h1>
        ...
        <div class="section">
            ...
            <div id="signatureDisplay">
                <?php if (!empty($user['digital_signature'])): ?>
                    <div class="signature-display">
                        <img src="<?php echo htmlspecialchars($user['digital_signature']); ?>" alt="Digital Signature" id="currentSignatureImg">
                        <div class="signature-meta">
                            <div class="meta-item">
                                <i class="fas fa-calendar-check"></i>
                                <span>Signed on: <strong><?php echo $user['signature_date'] ? date('F d, Y h:i A', strtotime($user['signature_date'])) : 'Date unavailable'; ?></strong></span>
                            </div>
                            <div class="meta-item">
                ...
                <?php else: ?>
                    <div class="no-signature">
                        ...
                        <p>No signature on file. Create your digital signature below.</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <script>
        ...
        async function confirmClearSignature() {
            if (!confirm('Are you sure you want to remove your signature from the database? This action cannot be undone.')) {
                return;
            }
        ...
        function downloadSignature() {
            ...
            link.download = 'signature_<?php echo $user_id; ?>_' + new Date().getTime() + '.png';
            link.href = img.src;
            link.click();
        }
