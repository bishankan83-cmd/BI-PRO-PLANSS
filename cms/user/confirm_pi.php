<?php
// confirm_pi.php (Customer Portal)
session_start();
...
function getCurrencySymbol($currency_name) {
    $map = [
        'us dollar'         => 'USD $',
        'us dollars'        => 'USD $',
        'usd'               => 'USD $',
    ...
    $key = strtolower(trim($currency_name));
    return $map[$key] ?? $currency_name;
}
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['action']) && $_POST['action']==='confirm_order') {
    ...
    if (!empty($order_id)) {
        $verify_stmt = $conn->prepare("SELECT order_id FROM tire_orders WHERE order_id=? AND customer_id=?");
        if ($verify_stmt) {
            $verify_stmt->bind_param("si",$order_id,$user_id);
            $verify_stmt->execute();
            if ($verify_stmt->get_result()->num_rows > 0) {
                $q = "UPDATE tire_orders SET status='cus_pi_confirm',
...
$stmt = $conn->prepare("SELECT tor.*, u.*,
    u.id as user_id, u.cus_id, u.acm_name, u.acm_ref,
    u.fullName, u.company_rn, u.Country, u.registerd_Address,
    u.userEmail, u.delivery_address, u.contact_person1_name,
    u.payment_rate, u.digital_signature, u.signature_date,
    u.functional_currency AS user_functional_currency
...
$digital_signature  = $order['digital_signature'] ?? null;
$signature_date     = $order['signature_date'] ?? null;
...
// ── Functional currency (from users table, matched by cus_id) ─────────────────
// Priority: users.functional_currency → tire_orders.functional_currency → default
$functional_currency = 'US Dollar';

if (!empty($order['user_functional_currency'])) {
    $functional_currency = $order['user_functional_currency'];
} elseif (!empty($cus_id_for_rate)) {
    ...
    if ($fc_stmt) {
        ...
        } elseif (!empty($order['functional_currency'])) {
...
// Derive display symbol and words label from functional_currency
$currency_symbol = getCurrencySymbol($functional_currency);
$currency_words  = $functional_currency; // used verbatim in amount-in-words sentence
...
// ── Additional Charges (charge1–charge4) ──────────────────────────────────────
$charges = [];
for ($i = 1; $i <= 4; $i++) {
...
$amount_in_words = numberToWords($total, $currency_words);

$display_payment_rate = $order_payment_rate;
$rate_is_mixed = count($effective_rates_used) > 1;

// ── Signature: prefer ACM admin signature ─────────────────────────────────────
$admin_signatures = [];
...
// Safe HTML-escaped symbol for use in HTML output
$cs = htmlspecialchars($currency_symbol);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
td.col-actual{background:#fff;color:#000;font-weight:700;text-align:right}
th.col-disc{background:#2c2c2c!important;color:#fff!important}
td.col-disc{background:#fff;color:#000;font-weight:600;text-align:right}
th.col-rate{background:#2c2c2c!important;color:#fff!important}
td.col-rate{background:#fff;color:#000;font-weight:800;text-align:right;font-size:11.5px}
th.col-net{background:#2c2c2c!important;color:#fff!important}
td.col-net{background:#fff;color:#000;font-weight:800;text-align:right;font-size:11.5px}

td.col-actual.icode-price{background:#fff;color:#000;font-weight:700}
td.col-actual.brand-rate{background:#fff;color:#000}
...
.status-cus_pi_confirm,.status-pi_confirm{background:#cce5ff;color:#004085}
...
<div class="action-buttons">
    ...
    <?php if ($order['status'] !== 'cus_pi_confirm'): ?>
    ...
    <div class="invoice-content">
        ...
        <div class="invoice-details">
            <div class="detail-box">
                ...
                <div class="detail-row">
                    ...
                    <span class="detail-value"><?php echo htmlspecialchars($order['acm_ref'] ?: 'N/A'); ?></span>
                </div>
            ...
            <div class="detail-box">
                ...
                <div class="detail-row">
                    ...
                    <span class="detail-value"><?php echo htmlspecialchars($order['cus_id'] ?: $order['customer_id']); ?></span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Currency:</span>
                    <span class="detail-value">
                        <strong><?php echo $cs; ?></strong> &mdash; <?php echo htmlspecialchars($functional_currency); ?>
                    </span>
                </div>
                <div class="detail-row">
            ...
            <div class="summary-grid">
                ...
                <div class="summary-item">
                    ...
                    <div class="summary-value discount">
                        <?php echo $cs; ?><?php echo number_format($total_discount,2); ?>
                    </div>
                </div>
                <div class="summary-item">
                    ...
                    <?php else: ?>
                        <div class="summary-value">
                            <?php echo $cs; ?><?php echo number_format($display_payment_rate,4); ?>/kg
                        </div>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <table class="items-table">
            <thead>
                <tr class="thead-top">
                    ...
                    <th class="text-right col-actual" rowspan="2" style="vertical-align:middle;width:9%">
                        Actual Unit Price (<?php echo $cs; ?>)<br>
            ...
            <?php else: $c=1; foreach($items as $item):
                ...
                if ($is_icode_price)    $actual_cell_class = 'col-actual icode-price';
                elseif ($is_brand_rate) $actual_cell_class = 'col-actual brand-rate';
                else                    $actual_cell_class = 'col-actual';
            ?>
                <tr>
                    <td><?php echo $c++; ?></td>
                    ...
                    <td class="<?php echo $actual_cell_class; ?>">
                        <?php echo $cs; ?><?php echo number_format($item['actual_unit_price'],2); ?>
                    </td>
                    <td class="col-disc">
                        <?php if ($item['disc_pct'] > 0): ?>
                            <span style="display:block;font-size:10px;color:#000;margin-bottom:2px;font-weight:700">
                                <?php echo number_format($item['disc_pct'],2); ?>%
                            </span>
                        <?php endif; ?>
                        <?php if ($item['disc_n'] > 0): ?>
                            &minus; <?php echo $cs; ?><?php echo number_format($item['disc_n'],2); ?>
                        <?php else: ?>
                            <span style="color:#bbb">&mdash;</span>
                        <?php endif; ?>
                    </td>
                    <td class="col-rate">
                        <?php echo $cs; ?><?php echo number_format($item['discounted_price'],2); ?>
                    </td>
                    <td class="col-net">
                        <?php echo $cs; ?><?php echo number_format($item['net_amount'],2); ?>
                    </td>
                </tr>
        ...
        <div class="totals-section">
            ...
            <div class="totals">
                <div class="total-row">
                    ...
                    <span><?php echo $cs; ?><?php echo number_format($grand_before_discount,2); ?></span>
                </div>
                <div class="total-row discount-row">
                    <span>Total Discount:</span>
                    <span>&minus; <?php echo $cs; ?><?php echo number_format($total_discount,2); ?></span>
                </div>
                <div class="total-row subtotal">
                    <span>Sub Total <small>(after discount)</small>:</span>
                    <span><?php echo $cs; ?><?php echo number_format($subtotal,2); ?></span>
                </div>
                <div class="total-row">
                    <span>Total Tax Amount (0.00%):</span>
                    <span><?php echo $cs; ?><?php echo number_format($tax_amount,2); ?></span>
                </div>
                <div class="total-row">
                    <span>Carriage Amount:</span>
                    <span><?php echo $cs; ?><?php echo number_format($carriage,2); ?></span>
                </div>
                <?php if (!empty($charges)): ?>
                    <?php foreach ($charges as $ch): ?>
                    <div class="total-row charge-row">
                        <span><?php echo htmlspecialchars($ch['name']); ?>:</span>
                        <span><?php echo $cs; ?><?php echo number_format($ch['value'],2); ?></span>
                    </div>
                    <?php endforeach; ?>
                <?php endif; ?>
                <div class="total-row grand-total">
                    <span>Payment Due:</span>
                    <span><?php echo $cs; ?><?php echo number_format($total,2); ?></span>
                </div>
            </div>
        </div>
                ...
                <div class="signature-line customer-signature">
                    <div class="signature-placeholder">
                        <?php if (!empty($digital_signature)): ?>
                            <img src="<?php echo htmlspecialchars($digital_signature); ?>"
                                 alt="Customer Signature" class="signature-image">
                        <?php endif; ?>
                    </div>
                    <div class="signature-title">Customer Signature</div>
                    <?php if (!empty($signature_date)): ?>
                    <div class="signature-meta">Signed on: <?php echo date('d M Y H:i',strtotime($signature_date)); ?></div>
                    <?php else: ?>
...
function confirmOrder() {
    ...
    .then(data => {
        if (data.success) {
            ...
            if (badge) { badge.className='status-badge status-cus_pi_confirm'; badge.textContent='cus_pi_confirm'; }
...
console.log('Functional Currency (from users table): <?php echo addslashes($functional_currency); ?>');
console.log('Currency Symbol resolved: <?php echo addslashes($currency_symbol); ?>');
console.log('Payment Rate: <?php echo addslashes($currency_symbol); ?><?php echo number_format($order_payment_rate,4); ?>/kg');
console.log('Grand Total: <?php echo addslashes($currency_symbol); ?><?php echo number_format($total,2); ?>');
console.log('Total Discount: <?php echo addslashes($currency_symbol); ?><?php echo number_format($total_discount,2); ?>');
console.log('Total Weight: <?php echo number_format($total_weight,2); ?> kgs');
</script>
</body>
</html>