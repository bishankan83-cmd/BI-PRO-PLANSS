...
if (isset($_GET['ajax']) && $_GET['ajax'] === '1') {
    ...
    try {
        ...
        $hasActiveFilters =
            !empty($_GET['icode_select'])     ||
            !empty($_GET['tire_size_select']) ||
            (!empty($_GET['brand_select']) && $_GET['brand_select'] !== 'all') ||
        ...
        $filterMap = [
            'icode_select'     => 'r.icode',
            'tire_size_select' => $tireSizeCol,
            'col_select'       => 'r.col',
            'rim_select'       => 'r.rim',
        ];
        ...
        $sortOptions = [
            'code_asc'       => 'r.icode ASC',
            'code_desc'      => 'r.icode DESC',
            'brand_asc'      => "$brandCol ASC",
            'brand_desc'     => "$brandCol DESC",
            'size_asc'       => 'r.t_size ASC',
            'size_desc'      => 'r.t_size DESC',
            'tire_size_asc'  => "$tireSizeCol ASC",
            'tire_size_desc' => "$tireSizeCol DESC",
            'weight_asc'     => 't.fweight ASC',
            'weight_desc'    => 't.fweight DESC',
        ];
        $sortBy = (isset($_GET['sort']) && isset($sortOptions[$_GET['sort']]))
            ? $sortOptions[$_GET['sort']]
            : "$brandCol ASC, r.t_size ASC";

        $whereStr = empty($whereConditions) ? '' : ' WHERE ' . implode(' AND ', $whereConditions);

        if ($useOldTable) {
            $sql = "SELECT r.id, r.icode, r.t_size, r.brand, r.col, r.rim,
                           t.fweight, t.tire_size, t.cbm,
                           t.Brand AS tire_brand, t.Description AS t_description,
                           t.Colour AS t_colour, t.Type AS t_type
                    FROM realstock r
                    LEFT JOIN tire_details_old t ON r.icode = t.icode
                    {$whereStr}
                    ORDER BY {$sortBy}";
        } else {
            $sql = "SELECT r.id, r.icode, r.t_size, r.brand, r.col, r.rim,
        ...
        foreach ($inventory as &$row) {
            ...
            if ($useOldTable && empty($row['tire_brand'])) {
                $row['tire_brand'] = $row['brand'] ?? '';
            }
        }
        unset($row);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST'
    ...
    if (!empty($orderItems)) {
        try {
            ...
            $result     = $stmt->fetch();
            $newOrderId = ($result['max_id'] ?? 0) + 1;

            $orderNotes = $_POST['order_notes'] ?? '';
            $stmt = $pdo->prepare(
                "INSERT INTO tire_orders
                 (order_id,customer_id,status,total_items,total_quantity,total_weight,total_cbm,order_notes,created_at)
                 VALUES (?,?,'pending',?,?,?,?,?,NOW())"
            );
            ...
            foreach ($orderItems as $item) {
                if (!isset($item['id'], $item['icode'], $item['quantity'])
                ...
                $unitWeight      = $item['fweight']  ?? 0;
                $totalItemWeight = $item['quantity']  * $unitWeight;
                $unitCBM         = $item['cbm']       ?? 0;
                $totalItemCBM    = $item['quantity']  * $unitCBM;

                $stmt = $pdo->prepare(
                    "INSERT INTO tire_order_items
                     (order_id,product_id,icode,quantity,unit_weight,total_weight,unit_cbm,total_cbm)
                     VALUES (?,?,?,?,?,?,?,?)"
                );
                $stmt->execute([
                    $newOrderId, $item['id'], $item['icode'], (int)$item['quantity'],
                    $unitWeight, $totalItemWeight, $unitCBM, $totalItemCBM,
                ]);

                $orderSummary[] =
                    "{$item['icode']} ({$item['brand']} - {$item['size']}): "
                    . "{$item['quantity']} units, Weight: "
                    . number_format($totalItemWeight, 2) . " kg, CBM: "
                    . number_format($totalItemCBM, 3);
            }

            $summaryDetails = implode("\n", $orderSummary);
            $stmt = $pdo->prepare(
                "INSERT INTO order_summaries
                 (order_id,user_id,total_items,total_quantity,total_weight,total_cbm,summary_details,created_at)
                 VALUES (?,?,?,?,?,?,?,NOW())"
            );
            $stmt->execute([
                $newOrderId, $userId,
                count($orderItems), $totalQuantity, $totalWeight, $totalCBM, $summaryDetails,
            ]);
            ...
            header('Location: order_review.php?order_id=' . $newOrderId);
            exit;
        } catch (Exception $e) {
            $pdo->rollBack();
...
try {
    $stmt = $pdo->prepare(
        "SELECT fullName, userEmail, functional_currency FROM users WHERE id=?"
    );
    $stmt->execute([$userId]);
    $userData = $stmt->fetch();
    ...
    $stmt     = $pdo->query("SELECT DISTINCT r.icode FROM realstock r WHERE r.icode IS NOT NULL AND r.icode != '' ORDER BY r.icode");
    $icodes   = array_column($stmt->fetchAll(), 'icode');

    $stmt      = $pdo->query("SELECT DISTINCT t.tire_size FROM tire_details t WHERE t.tire_size IS NOT NULL AND t.tire_size != '' ORDER BY t.tire_size");
    $tireSizes = array_column($stmt->fetchAll(), 'tire_size');

    if ($hasBrandRestriction && !empty($allowedBrands)) {
        $phs   = implode(',', array_fill(0, count($allowedBrands), '?'));
        $bStmt = $pdo->prepare(
            "SELECT DISTINCT t.brand FROM tire_details t
             WHERE t.brand IN ($phs)
    ...
    $stmt   = $pdo->query("SELECT DISTINCT col FROM realstock WHERE col IS NOT NULL AND col != '' ORDER BY col");
    $colors = array_column($stmt->fetchAll(), 'col');

    $stmt = $pdo->query("SELECT DISTINCT rim FROM realstock WHERE rim IS NOT NULL AND rim != '' ORDER BY rim");
    $rims = array_column($stmt->fetchAll(), 'rim');
...
/* ════════════════════════════════════════════════════════════════════════════
   URL PARAMETER PRE-LOAD  (?items=[{"icode":"152001","qty":10},…])
   Also supports ?source=preview and ?original_oid=63
   ════════════════════════════════════════════════════════════════════════════ */
$urlPreloadItems   = [];   // resolved rows ready to inject into JS
$urlPreloadErrors  = [];   // icodes that could not be found
$urlSourceLabel    = '';   // e.g. "preview" from ?source=preview
$urlOriginalOid    = null; // e.g. 63 from ?original_oid=63

if (!empty($_GET['items']) && $pdo) {
    $rawItems = json_decode(urldecode($_GET['items']), true);
    if (is_array($rawItems)) {

        // Build an indexed map of qty by icode (case-insensitive)
        $qtyMap = [];
        foreach ($rawItems as $ri) {
            $ic = strtolower(trim($ri['icode'] ?? ''));
            if ($ic && isset($ri['qty']) && (int)$ri['qty'] > 0) {
                $qtyMap[$ic] = (int)$ri['qty'];
            }
        }

        if (!empty($qtyMap)) {
            // Fetch all matching inventory rows in one query
            $phs  = implode(',', array_fill(0, count($qtyMap), '?'));
            $sql  = "SELECT r.id, r.icode, r.t_size, r.brand, r.col, r.rim,
                            t.fweight, t.tire_size, t.cbm, t.brand AS tire_brand
                     FROM realstock r
                     LEFT JOIN tire_details t ON r.icode = t.icode
                     WHERE LOWER(r.icode) IN ($phs)";
            try {
                $st = $pdo->prepare($sql);
                $st->execute(array_keys($qtyMap));
                $foundRows = $st->fetchAll();

                $foundCodes = [];
                foreach ($foundRows as $fr) {
                    $ic  = strtolower(trim($fr['icode'] ?? ''));
                    $ik  = $ic;
                    $cp  = isset($itemPrices[$ik]) ? $itemPrices[$ik] : null;
                    $br  = $fr['tire_brand'] ?? $fr['brand'] ?? '';
                    $urlPreloadItems[] = [
                        'id'            => $fr['id'],
                        'icode'         => $fr['icode'],
                        'size'          => $fr['t_size'] ?? '',
                        'tiresize'      => $fr['tire_size'] ?? '',
                        'brand'         => $br,
                        'color'         => $fr['col'] ?? '',
                        'rim'           => $fr['rim'] ?? '',
                        'fweight'       => (float)($fr['fweight'] ?? 0),
                        'cbm'           => (float)($fr['cbm'] ?? 0),
                        'customerPrice' => $cp !== null ? (float)$cp : '',
                        'quantity'      => $qtyMap[$ic] ?? 1,
                        'approxAcknowledged' => ($cusHasItemPrices && $cp === null) ? false : true,
                    ];
                    $foundCodes[] = $ic;
                }

                // Report any icodes that weren't found
                foreach (array_keys($qtyMap) as $ic) {
                    if (!in_array($ic, $foundCodes)) {
                        $urlPreloadErrors[] = $ic;
                    }
                }
            } catch (PDOException $e) {
                // Silently skip pre-load on DB error
            }
        }
    }
}
...
if (!empty($_GET['source'])) {
    $urlSourceLabel = htmlspecialchars(trim($_GET['source']));
}
if (!empty($_GET['original_oid']) && is_numeric($_GET['original_oid'])) {
    $urlOriginalOid = (int)$_GET['original_oid'];
}

$initials = strtoupper(substr($userData['fullName'] ?? 'U', 0, 1));
if (strpos($userData['fullName'] ?? '', ' ') !== false) {
    $initials .= strtoupper(
        substr($userData['fullName'], strpos($userData['fullName'], ' ') + 1, 1)
    );
}
?>
<!DOCTYPE html>
...
:root{
    ...
    --trans:0.18s cubic-bezier(0.4,0,0.2,1);--hdr-h:56px;--dock-h:0px;
}
...
/* ═══ PRELOAD BANNER ════════════════════════════════════════════════════════ */
.preload-banner{display:flex;align-items:flex-start;gap:12px;padding:.8rem 1.1rem;background:linear-gradient(90deg,rgba(242,128,24,0.10) 0%,rgba(242,128,24,0.04) 100%);border-bottom:2px solid rgba(242,128,24,0.30);font-size:12.5px;flex-wrap:wrap}
.preload-banner-icon{width:34px;height:34px;border-radius:var(--radius-sm);background:var(--orange);color:#fff;display:flex;align-items:center;justify-content:center;font-size:13px;flex-shrink:0;box-shadow:0 3px 10px rgba(242,128,24,0.30)}
.preload-banner-body{flex:1;min-width:180px}
.preload-banner-title{font-weight:900;color:var(--gray-900);font-size:13px;margin-bottom:3px;display:flex;align-items:center;gap:7px}
.preload-banner-sub{color:#7a4400;font-weight:500;font-size:11.5px;line-height:1.5}
.preload-banner-pills{display:flex;flex-wrap:wrap;gap:4px;margin-top:6px}
.preload-pill{display:inline-flex;align-items:center;gap:4px;padding:3px 9px;background:#fff;border:1.5px solid rgba(242,128,24,0.35);border-radius:20px;font-size:10.5px;font-weight:800;color:var(--orange)}
.preload-pill-qty{background:var(--orange);color:#fff;border-radius:10px;padding:0 5px;font-size:9px;font-weight:900}
.preload-error-pill{display:inline-flex;align-items:center;gap:4px;padding:3px 9px;background:#fff0f0;border:1.5px solid #f8a4a4;border-radius:20px;font-size:10.5px;font-weight:800;color:#b91c1c}
.preload-banner-src{display:inline-flex;align-items:center;gap:5px;padding:3px 10px;background:rgba(242,128,24,0.12);border:1px solid rgba(242,128,24,0.25);border-radius:20px;font-size:10px;font-weight:800;color:#7a4400;letter-spacing:.05em;text-transform:uppercase;margin-left:auto;align-self:flex-start;white-space:nowrap;flex-shrink:0}
...
@media(max-width:960px){
    .left-sidebar{
        position:fixed;top:0;left:0;height:100%;z-index:460;
        transform:translateX(-100%);width:min(var(--sidebar-w), 85vw);
        box-shadow:var(--shadow-lg);padding-top:var(--hdr-h);
    }
...
<!-- ═══ URL PRE-LOAD BANNER ══════════════════════════════════════════════ -->
<?php if (!empty($urlPreloadItems) || !empty($urlPreloadErrors)): ?>
<div class="preload-banner" id="preloadBanner">
    <div class="preload-banner-icon"><i class="fas fa-bolt"></i></div>
    <div class="preload-banner-body">
        <div class="preload-banner-title">
            <i class="fas fa-check-circle" style="color:var(--orange);font-size:11px;"></i>
            Order Pre-loaded from Link
            <?php if ($urlOriginalOid): ?>
                <span style="background:rgba(242,128,24,0.12);color:var(--orange);border-radius:20px;padding:1px 9px;font-size:9px;font-weight:900;letter-spacing:.07em;">
                    Order #<?php echo $urlOriginalOid; ?>
                </span>
            <?php endif; ?>
        </div>
        <div class="preload-banner-sub">
            <?php if (!empty($urlPreloadItems)): ?>
                <?php echo count($urlPreloadItems); ?> item<?php echo count($urlPreloadItems) !== 1 ? 's' : ''; ?> automatically added to your cart.
                Review quantities below and click <strong>Place Order</strong> when ready.
            <?php endif; ?>
            <?php if (!empty($urlPreloadErrors)): ?>
                <span style="color:#b91c1c;font-weight:700;margin-left:4px;">
                    <i class="fas fa-exclamation-triangle"></i>
                    <?php echo count($urlPreloadErrors); ?> item code<?php echo count($urlPreloadErrors) !== 1 ? 's' : ''; ?> not found in inventory.
                </span>
            <?php endif; ?>
        </div>
        <div class="preload-banner-pills">
            <?php foreach ($urlPreloadItems as $pi): ?>
                <span class="preload-pill">
                    <i class="fas fa-barcode" style="font-size:8px;"></i>
                    <?php echo htmlspecialchars($pi['icode']); ?>
                    <span class="preload-pill-qty"><?php echo $pi['quantity']; ?></span>
                </span>
            <?php endforeach; ?>
            <?php foreach ($urlPreloadErrors as $ec): ?>
                <span class="preload-error-pill">
                    <i class="fas fa-times-circle" style="font-size:8px;"></i>
                    <?php echo htmlspecialchars($ec); ?> — not found
                </span>
            <?php endforeach; ?>
        </div>
    </div>
    <?php if ($urlSourceLabel): ?>
        <div class="preload-banner-src"><i class="fas fa-link" style="font-size:8px;"></i><?php echo $urlSourceLabel; ?></div>
    <?php endif; ?>
</div>
<?php endif; ?>
    ...
    <div class="page-body">
        <section class="inv-panel">
            ...
            <div class="old-table-banner" id="oldTableBanner">
                ...
                Viewing <strong>Existing Tires</strong> — click toggle to return to New Tires.
            </div>
...
/* ── URL pre-load data (resolved server-side) ─────────────────────────── */
const urlPreloadData = <?php echo json_encode($urlPreloadItems); ?>;
...
function syncBrandUI() {
    ...
    if (count === 0) {
        textEl.textContent = 'All Brands';
        displayEl.classList.remove('active-filter');
        badgeEl.style.display = 'none';
    } else {
        textEl.textContent = count === 1 ? [...selectedBrands][0] : count + ' brands selected';
        displayEl.classList.add('active-filter');
        badgeEl.style.display = 'inline';
        badgeEl.textContent = count;
    }
    document.getElementById('brandMultiCountLabel').textContent = count + ' selected';
    pillsEl.innerHTML = [...selectedBrands].map(b =>
        `<div class="brand-pill" onclick="removeBrand('${esc(b)}')"><i class="fas fa-times"></i>${esc(b)}</div>`
    ).join('');
    updateFilterUI();
}
...
$(document).ready(function() {
    ...
    /* ── URL pre-load: inject items from ?items= parameter ────────────── */
    if (urlPreloadData && urlPreloadData.length > 0) {
        // Don't load from sessionStorage when URL pre-load is present
        // so fresh link data always wins
        injectUrlPreload();
    } else {
        loadState();
    }
...
/**
 * injectUrlPreload
 * Populates orderItems from server-resolved urlPreloadData, then
 * syncs quantity inputs and refreshes the summary + containers.
 */
function injectUrlPreload() {
    orderItems.clear();

    urlPreloadData.forEach(function(item) {
        // Build the orderItems entry exactly as _doUpdateOrder would
        orderItems.set(String(item.id), {
            id:                 String(item.id),
            icode:              item.icode,
            size:               item.size,
            ...
            color:              item.color,
            rim:                item.rim,
            fweight:            parseFloat(item.fweight) || 0,
            cbm:                parseFloat(item.cbm) || 0,
            quantity:           parseInt(item.quantity) || 1,
            customerPrice:      item.customerPrice !== null && item.customerPrice !== undefined
                                    ? String(item.customerPrice) : '',
            approxAcknowledged: item.approxAcknowledged ? true : false,
        });

        // Sync any matching qty inputs that are already rendered in the DOM
        const id = String(item.id);
        const qty = parseInt(item.quantity) || 1;
        document.querySelectorAll('.qty-inp[data-id="' + id + '"]').forEach(function(inp) {
            inp.value = qty;
            inp.classList.add('filled');
            inp.dataset.approxAcknowledged = '1';
            const row   = inp.closest('tr');
            const mcard = inp.closest('.m-card');
    ...
    // Open the dock immediately so the user sees what was pre-loaded
    if (orderItems.size > 0 && !dockOpen) {
        dockOpen = true;
        document.getElementById('dockBody').classList.add('open');
        document.getElementById('dockIcon').className = 'fas fa-chevron-down';
    }
}
...
/* ── Table toggle (New Tires / Existing Tires) ───────────────────────── */
function toggleTable() {
    ...
    if (usingOldTable) {
        btn.classList.add('active-old');
        label.textContent = 'Existing Tires';
        if (mToggle) { mToggle.classList.add('active-old'); }
        if (mLabel)  { mLabel.textContent = 'Existing'; }
        banner.classList.add('show');
    } else {
        btn.classList.remove('active-old');
        label.textContent = 'New Tires';
        if (mToggle) { mToggle.classList.remove('active-old'); }
        if (mLabel)  { mLabel.textContent = 'New Tires'; }
...
function updateFilterUI() {
    const fields = [
        { id: 'icode_select', label: 'Code' },
        { id: 'tire_size_select', label: 'Size' },
        { id: 'col_select', label: 'Color' },
        { id: 'rim_select', label: 'Rim' }
    ];
    ...
    fields.forEach(f => {
        ...
        if (active) {
            el.classList.add('active-filter'); count++;
            chips.push(`<div class="filter-chip" onclick="clearOne('${f.id}')"><i class="fas fa-times"></i>${esc(v)}</div>`);
        } else {
    ...
    fetch('?' + p.toString())
        ...
        .then(d => {
            if (d.success) {
                ...
                // After re-render, re-apply qty values from orderItems to newly rendered inputs
                restoreQtyInputs();
            }
        })
        .catch(console.error)
...
/**
 * restoreQtyInputs
 * After any table re-render (AJAX search, table toggle), re-apply
 * quantities stored in orderItems to matching DOM inputs.
 */
function restoreQtyInputs() {
    orderItems.forEach(function(item, id) {
        document.querySelectorAll('.qty-inp[data-id="' + id + '"]').forEach(function(inp) {
            if (parseInt(inp.value) !== item.quantity) {
...
function renderTable(items, isOld) {
    ...
    // Pre-build saved qty map from current orderItems (not DOM, since DOM is being replaced)
    const saved = {};
    orderItems.forEach((item, id) => { saved[id] = item.quantity; });
    ...
    brandGroups.forEach((groupItems, brandName) => {
        ...
        groupItems.forEach(item => {
            const savedQty = saved[item.id] || 0;
            const hv  = savedQty > 0 ? 'filled' : '';
            ...
            const rimVal  = item.rim || '—';

            // If item is already in orderItems (pre-loaded or previously selected), always ack
            const alreadyInOrder = orderItems.has(String(item.id));
            const ackAttr = (alreadyInOrder || !needsApprox) ? '1' : '0';
            ...
            const qtyAttr   = `data-id="${esc(item.id)}" data-icode="${esc(item.icode || '')}" data-size="${esc(descVal)}" data-tiresize="${esc(item.tire_size || '')}" data-brand="${esc(br)}" data-color="${esc(colVal)}" data-rim="${esc(rimVal)}" data-fweight="${esc(item.fweight || 0)}" data-cbm="${esc(item.cbm || 0)}" data-customer-price="${esc(cp)}" data-needs-approx-warning="${needsApprox ? '1' : '0'}" data-approx-acknowledged="${ackAttr}"`;

            tableRows += `<tr data-id="${esc(item.id)}" class="${savedQty > 0 ? 'selected' : ''}">
                ...
                <td><input type="number" class="qty-inp ${hv}" min="0" value="${savedQty}" placeholder="0" ${qtyAttr}
                    onchange="updateOrder(this)" oninput="updateOrder(this)"></td>
            </tr>`;

            mobileCards += `<div class="m-card${savedQty > 0 ? ' selected' : ''}" data-id="${esc(item.id)}">
                <div class="m-card-header">
                    <div>
                ...
                <div class="m-card-footer">
                    ...
                    <div class="m-qty-wrap">
                        <button type="button" class="m-qty-btn" onclick="mQtyAdj(this,-1)"><i class="fas fa-minus" style="font-size:10px;"></i></button>
                        <input type="number" class="m-qty-inp qty-inp ${hv}" min="0" value="${savedQty}" placeholder="0" ${qtyAttr}
                            onchange="updateOrder(this)" oninput="updateOrder(this)">
                        <button type="button" class="m-qty-btn" onclick="mQtyAdj(this,1)"><i class="fas fa-plus" style="font-size:10px;"></i></button>
                    </div>
                </div>
