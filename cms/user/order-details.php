...
$userId  = $_SESSION['id'];
if (empty($orderId)) { header('location:view_order.php'); exit(); }
...
// UPDATED: computed_total_payment = effective_unit_price × qty × (1 - discount/100)
function fetchItems($con, $oid) {
    ...
    $clean = fn($v) => (float) preg_replace('/[^0-9.]/', '', $v ?? '');
    ...
    if ($r) {
        while ($row = mysqli_fetch_assoc($r)) {
            ...
            $rate  = ($row['rate_value'] !== null && $row['rate_value'] !== '')
                     ? (float) $row['rate_value']
                     : null;

            // Effective unit price: prefer rate_value, fall back to unit_price
            $effectivePrice = ($rate !== null) ? $rate : $price;

            // Discount as a percentage (e.g. 10 means 10%)
            $discountPct = (float) preg_replace('/[^0-9.]/', '', $row['discount'] ?? '0');

            $unitWeight  = $clean($row['unit_weight']);
            $totalWeight = $clean($row['total_weight']);
            $unitCbm     = $clean($row['unit_cbm']);
            $totalCbm    = $clean($row['total_cbm']);
            $payAmt      = $clean($row['payment_amount']);
            $totalPay    = $clean($row['total_payment']);

            // ── KEY CALCULATION ──────────────────────────────────────────
            // Total Payment = Effective Unit Price × Qty × (1 − Discount%)
            $computedTotalPayment = $effectivePrice * $qty * (1 - $discountPct / 100);
            // Discount amount in dollars (for display)
            $discountAmount       = $effectivePrice * $qty * ($discountPct / 100);
            ...
            $row['rate_value_numeric']        = $rate;
            $row['effective_price_numeric']   = $effectivePrice;
            $row['discount_pct']              = $discountPct;
            $row['discount_amount']           = $discountAmount;
            $row['unit_weight_numeric']       = $unitWeight;
            $row['total_weight_numeric']      = $totalWeight;
            $row['unit_cbm_numeric']          = $unitCbm;
            $row['total_cbm_numeric']         = $totalCbm;
            $row['payment_amount_numeric']    = $payAmt;
            $row['total_payment_numeric']     = $totalPay;              // raw DB value
            $row['computed_total_payment']    = $computedTotalPayment;  // ← calculated value
            $row['sub_total']                 = $subTotal;
            $row['total_price']               = $computedTotalPayment;  // used for order-value sums
...
// Helper: display effective unit price — prefer rate_value, fallback to unit_price
function displayUnitPrice($item) {
    if ($item['rate_value_numeric'] !== null) {
        return fmtMoney($item['rate_value_numeric']);
    }
    return $item['unit_price_numeric'] ? fmtMoney($item['unit_price_numeric']) : '—';
}

// Helper: display computed total payment with tooltip breakdown
function displayTotalPayment($item) {
    $val = $item['computed_total_payment'];
    if ($val <= 0 && $item['effective_price_numeric'] <= 0) return '—';
    return fmtMoney($val);
}
...
$isRevision      = ($order['is_revision'] === '1' || $order['is_revision'] === 1);
...
// UPDATED: use computed_total_payment for the order value sum
$totalOrderValue = array_sum(array_column($items, 'computed_total_payment'));
...
if ($isRevision && !empty($originalOrderId)) {
    ...
    $rvRes = mysqli_query($con, "
        ...
        WHERE original_order_id = '$safeOrigId'
          AND is_revision = '1'
...
$canReuse  = in_array($status, ['complete', 'share_planning']);
$canRevise = ($status !== 'pending' && $status !== '');
...
<head>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .btn-reuse{background:var(--white);color:var(--teal);border:1.5px solid var(--teal);box-shadow:var(--shadow-sm);}
        .btn-reuse:hover{background:var(--teal);color:var(--white);border-color:var(--teal);transform:translateY(-2px);box-shadow:0 6px 18px rgba(91,192,190,0.35);}
        .btn-revise{background:var(--white);color:var(--amber);border:1.5px solid var(--amber);box-shadow:var(--shadow-sm);}
        .btn-revise:hover{background:var(--amber);color:var(--white);border-color:var(--amber);transform:translateY(-2px);box-shadow:0 6px 18px rgba(245,158,11,0.35);}
        ...
        /* ── Discount / payment cell helpers ── */
        .discount-pill{display:inline-flex;align-items:center;gap:.28rem;background:rgba(39,174,96,.10);color:var(--success);border-radius:.4rem;padding:.12rem .5rem;font-size:.76rem;font-weight:700;}
        .no-discount{color:var(--text-gray);font-size:.82rem;}
        .payment-cell{display:flex;flex-direction:column;gap:.08rem;}
        .payment-cell .main-val{font-weight:700;color:var(--dark-gray);}
        .payment-cell .sub-val{font-size:.72rem;color:var(--text-gray);}
        .payment-cell .sub-val s{color:#bbb;}
...
<div class="page-header">
    ...
    <div class="header-actions-right">
        <?php if ($canReuse): ?>
            <a href="edit_order_enter.php?oid=<?php echo urlencode($orderId); ?>" class="btn btn-reuse">
                <i class="fas fa-redo"></i> Reuse Order
            </a>
        <?php endif; ?>
        <?php if ($canRevise): ?>
            <a href="revise_orders.php?oid=<?php echo urlencode($orderId); ?>" class="btn btn-revise">
                <i class="fas fa-edit"></i> Revise Order
            </a>
        <?php endif; ?>
...
<div class="card animate-in">
    <div class="card-header">
        ...
        <div style="display:flex;align-items:center;gap:.75rem;flex-wrap:wrap;">
            ...
            <?php if ($canReuse): ?>
            <a href="edit_order_enter.php?oid=<?php echo urlencode($orderId); ?>"
               class="btn btn-reuse" style="padding:.35rem 1rem;font-size:.78rem;border-radius:.5rem;">
                <i class="fas fa-redo" style="font-size:.72rem;"></i> Reuse
            </a>
            <?php endif; ?>
            <?php if ($canRevise): ?>
            <a href="revise_orders.php?oid=<?php echo urlencode($orderId); ?>"
               class="btn btn-revise" style="padding:.35rem 1rem;font-size:.78rem;border-radius:.5rem;">
                <i class="fas fa-edit" style="font-size:.72rem;"></i> Revise
            </a>
            <?php endif; ?>
        </div>
    </div>
    <div class="info-grid">
        ...
        <div class="info-row">
            ...
            <span class="info-value"><?php echo htmlentities($order['name']); ?></span>
        </div>
        <div class="info-row">
            <span class="info-label">Order Date</span>
            <span class="info-value"><?php echo fmtDate($order['order_date']); ?></span>
        </div>
        <div class="info-row">
            <span class="info-label">Order Reference</span>
            <span class="info-value"><?php echo !empty($order['order_reference']) ? htmlentities($order['order_reference']) : '<span class="no-val">—</span>'; ?></span>
        </div>
        <?php if (!empty($order['customer_reference'])): ?>
        <div class="info-row">
            <span class="info-label">Customer Reference</span>
            <span class="info-value"><?php echo htmlentities($order['customer_reference']); ?></span>
        </div>
        <?php endif; ?>
        <div class="info-row">
            <span class="info-label">Invoice No.</span>
            <span class="info-value"><?php echo !empty($order['invoice_no']) ? htmlentities($order['invoice_no']) : '<span class="no-val">—</span>'; ?></span>
        </div>
        <?php if (!empty($order['erp_number'])): ?>
        <div class="info-row">
            <span class="info-label">ERP Number</span>
            <span class="info-value"><?php echo htmlentities($order['erp_number']); ?></span>
        </div>
        <?php endif; ?>
        <div class="info-row">
            <span class="info-label">Total Items</span>
            <span class="info-value"><?php echo (int)$order['total_items']; ?></span>
        </div>
        <div class="info-row">
        ...
        <div class="info-row">
            ...
            <span class="info-value"><?php echo fmtKg($order['total_weight']); ?></span>
        </div>
        <div class="info-row">
            <span class="info-label">Total CBM</span>
            <span class="info-value"><?php echo !empty($order['total_cbm']) ? fmtNum($order['total_cbm'],4).' m³' : '<span class="no-val">—</span>'; ?></span>
        </div>
        <div class="info-row">
            <span class="info-label">Total Payment</span>
            <span class="info-value" style="color:var(--primary-orange);font-weight:700;">
                <?php
                // Show computed total payment (sum of all items' computed_total_payment)
                if ($totalOrderValue > 0):
                    echo fmtMoney($totalOrderValue);
                elseif (!empty($order['total_payment'])):
                    echo fmtMoney($order['total_payment']);
                else:
                    echo '<span class="no-val" style="color:var(--text-gray);font-weight:400;">—</span>';
                endif;
                ?>
            </span>
        </div>
        <?php if (!empty($order['request_status'])): ?>
        <div class="info-row">
            <span class="info-label">Request Status</span>
            <span class="info-value"><?php echo htmlentities($order['request_status']); ?></span>
        </div>
        <?php endif; ?>
        ...
        <div class="info-row">
            ...
            <span class="info-value"><?php echo htmlentities($order['plate']); ?></span>
        </div>
        <?php endif; ?>
        <?php if ($isRevision): ?>
        <div class="info-row">
            ...
            <span class="info-value">
                <a href="?oid=<?php echo urlencode($originalOrderId); ?>" style="color:var(--primary-orange);font-weight:600;">
                    #<?php echo htmlentities($originalOrderId); ?>
                </a>
            </span>
        </div>
        <?php endif; ?>
        ...
        <div class="info-row">
            <span class="info-label">Revised To</span>
            <span class="info-value">
                <a href="?oid=<?php echo urlencode($order['revised_order_id']); ?>" style="color:var(--revised);font-weight:600;">
                    #<?php echo htmlentities($order['revised_order_id']); ?> <i class="fas fa-external-link-alt fa-xs"></i>
                </a>
            </span>
        </div>
        <?php endif; ?>
        <?php if (!empty($order['accepted_by'])): ?>
        <div class="info-row">
            <span class="info-label">Accepted By</span>
            <span class="info-value"><?php echo htmlentities($order['accepted_by']); ?></span>
        </div>
        <?php endif; ?>
        <?php if (!empty($order['accepted_at'])): ?>
        <div class="info-row">
            <span class="info-label">Accepted At</span>
            <span class="info-value"><?php echo fmtDate($order['accepted_at']); ?></span>
        </div>
        <?php endif; ?>
        <?php if (!empty($order['confirmed_at'])): ?>
        <div class="info-row">
            <span class="info-label">Confirmed At</span>
            <span class="info-value"><?php echo fmtDate($order['confirmed_at']); ?></span>
        </div>
        <?php endif; ?>
        <?php if (!empty($order['created_at'])): ?>
        <div class="info-row">
            <span class="info-label">Created At</span>
            <span class="info-value"><?php echo fmtDateShort($order['created_at']); ?></span>
        </div>
        <?php endif; ?>
        <?php if (!empty($order['updated_at'])): ?>
        <div class="info-row">
            <span class="info-label">Last Updated</span>
            <span class="info-value"><?php echo fmtDate($order['updated_at']); ?></span>
        </div>
        <?php endif; ?>
    </div>
</div>
...
<!-- ══ NOTES & COMMENTS ══════════════════════════════════════════════════════ -->
<?php
$hasNotes = !empty($order['order_notes'])
         || !empty($order['customer_comment'])
         || !empty($order['acm_comment'])
         || !empty($order['marketing_executive_comment']);
?>
<?php if ($hasNotes): ?>
<div class="card animate-in">
    ...
    <div class="card-body" style="display:flex;flex-direction:column;gap:1.25rem;">
        ...
        <div>
            <div class="info-label" style="margin-bottom:.4rem;">Order Notes</div>
            <div class="notes-box"><?php echo htmlentities($order['order_notes']); ?></div>
        </div>
        <?php endif; ?>
        <?php if (!empty($order['customer_comment'])): ?>
        <div>
            <div class="info-label" style="margin-bottom:.4rem;">Customer Comment</div>
            <div class="notes-box"><?php echo htmlentities($order['customer_comment']); ?></div>
        </div>
        <?php endif; ?>
        <?php if (!empty($order['acm_comment'])): ?>
        <div>
            <div class="info-label" style="margin-bottom:.4rem;">ACM Comment</div>
            <div class="notes-box"><?php echo htmlentities($order['acm_comment']); ?></div>
        </div>
        <?php endif; ?>
        <?php if (!empty($order['marketing_executive_comment'])): ?>
        <div>
            <div class="info-label" style="margin-bottom:.4rem;">Marketing Executive Comment</div>
            <div class="notes-box"><?php echo htmlentities($order['marketing_executive_comment']); ?></div>
        </div>
        <?php endif; ?>
    </div>
</div>
...
<div class="card animate-in">
        ...
        <div class="ship-steps">
            ...
            <div class="ship-step <?php echo $cls; ?>">
                <div class="ship-step-dot">
                    ...
                    <?php else: ?>
                        <i class="fas <?php echo $st['icon']; ?>" style="font-size:.6rem;"></i>
                    <?php endif; ?>
                </div>
    ...
    <div class="ship-info-grid">
        ...
        <div class="ship-info-row">
            ...
            <span class="ship-info-value">
                <?php echo !empty($shipment['loading_date'])
                    ? '<span class="date-highlight"><i class="fas fa-calendar-check"></i>'.fmtDateShort($shipment['loading_date']).'</span>'
        ...
        <div class="ship-info-row">
            ...
            <span class="ship-info-value">
                <?php echo !empty($shipment['on_board_date'])
                    ? '<span class="date-highlight"><i class="fas fa-anchor"></i>'.fmtDateShort($shipment['on_board_date']).'</span>'
                    : '<span class="no-val">—</span>'; ?>
            </span>
        </div>
    </div>
        ...
        <div class="ship-info-row">
            ...
            <span class="ship-info-value">
                ...
                <span class="date-highlight <?php echo $etaClass; ?>">
                    ...
                    <?php if ($shipment['eta'] < $today): ?>
                        <span style="font-size:.72rem;color:var(--success);font-weight:600;margin-left:.3rem;"><i class="fas fa-check-circle"></i> Arrived</span>
                    <?php elseif ($shipment['eta'] <= date('Y-m-d', strtotime('+7 days'))): ?>
                        <span style="font-size:.72rem;color:var(--warning);font-weight:600;margin-left:.3rem;"><i class="fas fa-exclamation-triangle"></i> Soon</span>
                    <?php endif; ?>
                </span>
                <?php else: ?><span class="no-val">—</span><?php endif; ?>
            </span>
        ...
        <div class="ship-info-row">
            ...
            <span class="ship-info-value">
                ...
                <span class="date-highlight <?php echo $ddpClass; ?>">
                    ...
                    <?php if ($shipment['ddp_expected_date'] < $today): ?>
...
<div class="card animate-in">
    <div class="card-header">
        ...
        <span style="font-size:.8rem;color:var(--text-gray);">
            Total Payment = Unit Price &times; Qty &times; (1 &minus; Discount%)
        </span>
    </div>
    <div class="card-body" style="padding:0;">
        <div class="table-responsive">
            <table class="table">
                ...
                <tr>
                    ...
                    <td><?php echo displayUnitPrice($item); ?></td>
                    <!-- Sub-total before discount -->
                    <td style="color:var(--text-gray);"><?php echo $item['effective_price_numeric'] > 0 ? fmtMoney($item['sub_total']) : '—'; ?></td>
                    <!-- Discount with amount saved -->
                    <td>
                        <?php if ($item['discount_pct'] > 0): ?>
                            <div class="discount-pill">
                                <i class="fas fa-tag" style="font-size:.65rem;"></i>
                                <?php echo fmtNum($item['discount_pct'], 2); ?>%
                            </div>
                            <div style="font-size:.72rem;color:var(--success);margin-top:.15rem;">
                                &minus;<?php echo fmtMoney($item['discount_amount']); ?>
                            </div>
                        <?php else: ?>
                            <span class="no-discount">—</span>
                        <?php endif; ?>
                    </td>
                    <td><?php echo $item['unit_weight_numeric'] ? fmtKg($item['unit_weight_numeric']) : '—'; ?></td>
                    <td><?php echo $item['total_weight_numeric'] ? fmtKg($item['total_weight_numeric']) : '—'; ?></td>
                    <td><?php echo $item['unit_cbm_numeric'] ? fmtNum($item['unit_cbm_numeric'],4).' m³' : '—'; ?></td>
                    <td><?php echo $item['total_cbm_numeric'] ? fmtNum($item['total_cbm_numeric'],4).' m³' : '—'; ?></td>
                    <!-- COMPUTED Total Payment -->
                    <td>
                        <div class="payment-cell">
                            <span class="main-val"><?php echo displayTotalPayment($item); ?></span>
                            <?php if ($item['discount_pct'] > 0): ?>
                            <span class="sub-val"><s><?php echo fmtMoney($item['sub_total']); ?></s></span>
                            <?php endif; ?>
                        </div>
                    </td>
                </tr>
        ...
        <?php if (!empty($items)):
            $sumQty      = array_sum(array_column($items, 'quantity_numeric'));
            $sumWeight   = array_sum(array_column($items, 'total_weight_numeric'));
            $sumSubTotal = array_sum(array_column($items, 'sub_total'));
            $sumDiscount = array_sum(array_column($items, 'discount_amount'));
            $sumPayment  = array_sum(array_column($items, 'computed_total_payment'));
        ?>
        <div style="padding:1.25rem 1.75rem;">
            <div class="totals-row">
                <div class="total-item">
                    <span class="total-label">Items</span>
                    <span class="total-value"><?php echo count($items); ?></span>
                </div>
                <div class="total-item">
                    <span class="total-label">Quantity</span>
                    <span class="total-value"><?php echo fmtNum($sumQty); ?> pcs</span>
                </div>
                <div class="total-item">
                    <span class="total-label">Weight</span>
                    <span class="total-value"><?php echo fmtKg($sumWeight); ?></span>
                </div>
                <div class="total-item">
                    <span class="total-label">Sub-Total</span>
                    <span class="total-value" style="color:var(--text-gray);"><?php echo fmtMoney($sumSubTotal); ?></span>
                </div>
                <?php if ($sumDiscount > 0): ?>
                <div class="total-item">
                    <span class="total-label">Total Discount</span>
                    <span class="total-value" style="color:var(--success);">&minus;<?php echo fmtMoney($sumDiscount); ?></span>
                </div>
                <?php endif; ?>
                <div class="total-item">
                    <span class="total-label">Total Payment</span>
                    <span class="total-value highlight"><?php echo fmtMoney($sumPayment); ?></span>
                </div>
            </div>
        </div>
        <?php endif; ?>
...
<div class="card animate-in">
    ...
    <div class="timeline">
        <?php foreach ($allRevisions as $idx => $rev):
            ...
            $pPayment  = array_sum(array_column($pItems,'computed_total_payment'));
        ?>
        <div class="timeline-item">
            <div class="timeline-dot <?php echo $dotClass; ?>"><?php echo $dotLabel; ?></div>
            <div class="timeline-content <?php echo $contClass; ?>"
                 onclick="switchRevTab(<?php echo $idx; ?>)"
                 title="View details for this version">
                <div class="timeline-top">
                    <span class="timeline-oid">
                        <?php if (!$isCurrent): ?>
                            <a href="?oid=<?php echo urlencode($rev['order_id']); ?>" onclick="event.stopPropagation();">
                    ...
                    <?php endif; ?>
                    <?php if ($isCurrent): ?>
                        <span class="timeline-current-tag"><i class="fas fa-eye" style="font-size:.6rem;"></i> Viewing</span>
                    <?php endif; ?>
                ...
                <div class="timeline-meta">
                    ...
                    <?php if ($pPayment > 0): ?>
                    <span>Payment: <strong><?php echo fmtMoney($pPayment); ?></strong></span>
                    <?php elseif (!empty($rev['order']['total_payment'])): ?>
                    ...
                    <?php endif; ?>
                    <?php if (!empty($rev['order']['erp_number'])): ?>
                    <span>ERP: <strong><?php echo htmlentities($rev['order']['erp_number']); ?></strong></span>
                    <?php endif; ?>
                    <?php if ($idx > 0):
...
<div class="card animate-in">
    ...
    <div class="rev-tabs-wrap">
        <div class="rev-tabs" id="revTabBar">
            <?php foreach ($allRevisions as $idx => $rev):
                ...
                $cls      = 'rev-tab'
                    . ($isActive ? ' active' : '')
                    . (!$rev['is_original'] ? ' is-revision-tab' : '')
                    . ($isActive && !$rev['is_original'] ? ' is-rev' : '');
            ?>
            <button class="<?php echo $cls; ?>" onclick="switchRevTab(<?php echo $idx; ?>)" id="revTab<?php echo $idx; ?>">
                <?php if ($rev['order_id'] == $orderId): ?>
                    <i class="fas fa-eye" style="font-size:.68rem;opacity:.7;margin-right:.18rem;"></i>
                <?php endif; ?>
    ...
    <?php foreach ($allRevisions as $idx => $rev):
        ...
        // UPDATED: use computed_total_payment for panel totals
        $panelSubTotal  = array_sum(array_column($panelItems,'sub_total'));
        $panelDiscount  = array_sum(array_column($panelItems,'discount_amount'));
        $panelTotalPay  = array_sum(array_column($panelItems,'computed_total_payment'));
    ...
    <div class="rev-panel <?php echo $isActive ? 'active' : ''; ?>" id="revPanel<?php echo $idx; ?>">
        ...
        <div style="padding:1.25rem 1.75rem 0;">
            ...
            <div class="comparison-grid" style="margin-bottom:1.25rem;">
                ...
                <div class="comparison-panel cmp-prev">
                    ...
                    <dl class="panel-meta">
                        <dt>Date</dt>
                        <dd><?php echo fmtDateShort($allRevisions[$idx-1]['order']['order_date']); ?></dd>
                        <dt>Items / Qty</dt>
                        <dd><?php echo (int)$allRevisions[$idx-1]['order']['total_items']; ?> items / <?php echo fmtNum(array_sum(array_column($prevItems,'quantity_numeric'))); ?> pcs</dd>
                        <dt>Weight</dt>
                        <dd><?php echo fmtKg($allRevisions[$idx-1]['order']['total_weight']); ?></dd>
                        <dt>Total Payment</dt>
                        <dd style="font-weight:700;color:var(--primary-orange);">
                            <?php $prevPay = array_sum(array_column($prevItems,'computed_total_payment'));
                            echo $prevPay > 0 ? fmtMoney($prevPay) : (!empty($allRevisions[$idx-1]['order']['total_payment']) ? fmtMoney($allRevisions[$idx-1]['order']['total_payment']) : '—'); ?>
                        </dd>
                    </dl>
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                <thead><tr>
                                    ...
                                    <th>Qty</th><th>Unit Price</th><th>Discount</th><th>Total Payment</th>
                                </tr></thead>
                                <tbody>
                                ...
                                <tr class="<?php echo $rc; ?>">
                                    ...
                                    <td><strong><?php echo htmlentities($pi['icode']); ?></strong>
                                        ...
                                        <?php elseif ($rc === 'row-changed'): ?><span class="change-pill pill-changed">Changed</span>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td><?php echo displayUnitPrice($pi); ?></td>
                                    <td><?php echo $pi['discount_pct'] > 0 ? '<span class="discount-pill">'.fmtNum($pi['discount_pct'],2).'%</span>' : '<span class="no-discount">—</span>'; ?></td>
                                    <td style="font-weight:700;"><?php echo displayTotalPayment($pi); ?></td>
                                </tr>
                                <?php endforeach; else: ?>
                                <tr><td colspan="7"><div class="empty-state"><i class="fas fa-inbox empty-icon"></i><p>No items</p></div></td></tr>
                                <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
                ...
                <div class="comparison-panel cmp-cur">
                    ...
                    <dl class="panel-meta">
                        <dt>Date</dt>
                        <dd><?php echo fmtDateShort($panelOrder['order_date']); ?></dd>
                        <dt>Items / Qty</dt>
                        <dd><?php echo (int)$panelOrder['total_items']; ?> items / <?php echo fmtNum($panelTotalQty); ?> pcs</dd>
                        <dt>Weight</dt>
                        <dd><?php echo fmtKg($panelOrder['total_weight']); ?></dd>
                        <dt>Total Payment</dt>
                        <dd style="font-weight:700;color:var(--primary-orange);"><?php echo $panelTotalPay > 0 ? fmtMoney($panelTotalPay) : '—'; ?></dd>
                    </dl>
                    <div style="padding:.75rem 0 .5rem;">
                        <div class="section-label"><i class="fas fa-list"></i> Items</div>
                        <div class="table-responsive">
                                ...
                                <tr class="<?php echo $rc; ?>">
                                    ...
                                    <td><strong><?php echo htmlentities($ci['icode']); ?></strong>
                                        ...
                                        <?php elseif ($rc === 'row-changed'): ?>
                                            <span class="change-pill pill-changed">
                                                <?php echo fmtNum($prevItemMap[$ci['icode']]['quantity_numeric']); ?> &rarr; <?php echo fmtNum($ci['quantity_numeric']); ?>
                                            </span>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td><?php echo displayUnitPrice($ci); ?></td>
                                    <td><?php echo $ci['discount_pct'] > 0 ? '<span class="discount-pill">'.fmtNum($ci['discount_pct'],2).'%</span>' : '<span class="no-discount">—</span>'; ?></td>
                                    <td style="font-weight:700;"><?php echo displayTotalPayment($ci); ?></td>
                                </tr>
                                <?php endforeach; else: ?>
                                <tr><td colspan="7"><div class="empty-state"><i class="fas fa-inbox empty-icon"></i><p>No items</p></div></td></tr>
                                <?php endif; ?>
        ...
        <!-- Full item list for this version -->
        <div style="padding:0 1.75rem 1.5rem;">
                ...
                <table class="table">
                    ...
                    <tr class="<?php echo $rc; ?>">
                        ...
                        <td><strong><?php echo htmlentities($item['icode']); ?></strong>
                            <?php if ($rc === 'row-added'): ?>
                                <span class="change-pill pill-added">New</span>
                            <?php elseif ($rc === 'row-changed'): ?>
                                <span class="change-pill pill-changed">
                                    <?php echo fmtNum($prevItemMap[$item['icode']]['quantity_numeric']); ?> &rarr; <?php echo fmtNum($item['quantity_numeric']); ?>
                                </span>
                            <?php endif; ?>
                        </td>
                        <td class="desc-cell"><?php echo !empty($item['description']) ? htmlentities($item['description']) : '<span class="no-val">—</span>'; ?></td>
                        ...
                        <td><?php echo displayUnitPrice($item); ?></td>
                        <!-- Sub-total before discount -->
                        <td style="color:var(--text-gray);font-size:.83rem;"><?php echo $item['effective_price_numeric'] > 0 ? fmtMoney($item['sub_total']) : '—'; ?></td>
                        <!-- Discount -->
                        <td>
                            <?php if ($item['discount_pct'] > 0): ?>
                                <div class="discount-pill">
                        ...
                        <td><?php echo $item['rate_value_numeric'] !== null ? fmtMoney($item['rate_value_numeric']) : '—'; ?></td>
                    </tr>
                    <?php endforeach; else: ?>
                    <tr><td colspan="13"><div class="empty-state"><div class="empty-icon"><i class="fas fa-inbox"></i></div><p>No items found.</p></div></td></tr>
                    <?php endif; ?>
                    </tbody>
                </table>
            </div>
            ...
            <div style="margin-top:1rem;">
                <div class="totals-row">
                    <div class="total-item">
                        <span class="total-label">Items</span>
                        <span class="total-value"><?php echo count($panelItems); ?></span>
                    </div>
                    <div class="total-item">
                        <span class="total-label">Quantity</span>
                        <span class="total-value"><?php echo fmtNum($panelTotalQty); ?> pcs</span>
                    </div>
                    <div class="total-item">
                        <span class="total-label">Weight</span>
                        <span class="total-value"><?php echo fmtKg($panelOrder['total_weight']); ?></span>
                    </div>
                    <div class="total-item">
                        <span class="total-label">Sub-Total</span>
                        <span class="total-value" style="color:var(--text-gray);"><?php echo fmtMoney($panelSubTotal); ?></span>
                    </div>
                    <?php if ($panelDiscount > 0): ?>
                    <div class="total-item">
                        <span class="total-label">Total Discount</span>
                        <span class="total-value" style="color:var(--success);">&minus;<?php echo fmtMoney($panelDiscount); ?></span>
                    </div>
                    <?php endif; ?>
                    <div class="total-item">
                        <span class="total-label">Total Payment</span>
                        <span class="total-value highlight"><?php echo fmtMoney($panelTotalPay); ?></span>
                    </div>
                </div>
            </div>
            <?php endif; ?>
...
<div id="deleteModal" class="modal">
    <div class="modal-content">
        ...
        <div class="modal-footer">
            ...
            <a href="?oid=<?php echo urlencode($orderId); ?>&delete=true" class="btn btn-danger">
                <i class="fas fa-trash-alt"></i> Yes, Delete Order
            </a>
        </div>
    </div>
</div>
