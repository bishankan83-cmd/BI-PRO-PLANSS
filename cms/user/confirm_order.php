...
/* ══ LOAD EXISTING ORDER FROM URL ══════════════════════════════════════ */
$existingOrderId = isset($_GET['id']) ? $_GET['id'] : null;
$existingOrderItems = [];
$existingOrder = null;
$loadingExistingOrder = false;

if ($existingOrderId && $pdo) {
    try {
        $stmt = $pdo->prepare("SELECT * FROM tire_orders WHERE order_id = ?");
        $stmt->execute([$existingOrderId]);
        $existingOrder = $stmt->fetch();
        if ($existingOrder) {
            $stmt = $pdo->prepare("
                SELECT toi.*, r.t_size, r.brand as realstock_brand, r.col, r.rim,
                       t.tire_size, t.brand as tire_brand, t.fweight, t.cbm
                FROM tire_order_items toi
                LEFT JOIN realstock r ON toi.product_id = r.id
                LEFT JOIN tire_details t ON toi.icode = t.icode
                WHERE toi.order_id = ?
            ");
            $stmt->execute([$existingOrderId]);
            $existingOrderItems = $stmt->fetchAll();
            $loadingExistingOrder = true;
            $message = "📋 Loaded Order #" . htmlspecialchars($existingOrderId) . " — You can modify quantities below";
            $messageType = 'info';
        } else {
            $message = "Order #" . htmlspecialchars($existingOrderId) . " not found";
            $messageType = 'error';
        }
    } catch (PDOException $e) {
        $message = "Error loading order: " . $e->getMessage(); $messageType = 'error';
    }
}
    ...
    try {
        ...
        if (!empty($_GET['brand_select']) && $_GET['brand_select']!=='all') {
            ...
            if (!empty($selectedBrandsFilter)) {
                ...
                $whereConditions[] = "t.brand IN ($phs2)";
                foreach ($selectedBrandsFilter as $sb) $params[] = $sb;
            }
        }
        $filterMap=['icode_select'=>'r.icode','tire_size_select'=>'t.tire_size','col_select'=>'r.col','rim_select'=>'r.rim'];
        ...
        unset($row);
        echo json_encode(['success'=>true,'data'=>$inventory,'count'=>count($inventory),'cusHasItemPrices'=>$ajaxCusHasItemPrices]);
        exit;
    } catch (PDOException $e) {
    ...
    if (!empty($orderItems)) {
        try {
            ...
            if ($existingOrderId && $existingOrder) {
                ...
                $stmt=$pdo->prepare("UPDATE tire_orders SET total_items=?,total_quantity=?,total_weight=?,total_cbm=?,order_notes=?,updated_at=NOW() WHERE order_id=?");
                $stmt->execute([count($orderItems),$totalQuantity,$totalWeight,$totalCBM,$orderNotes,$orderIdToUse]);
                $stmt=$pdo->prepare("DELETE FROM tire_order_items WHERE order_id=?");
                $stmt->execute([$orderIdToUse]);
            } else {
                $stmt=$pdo->query("SELECT MAX(order_id) as max_id FROM tire_orders");
                $result=$stmt->fetch(); $orderIdToUse=($result['max_id']??0)+1;
                ...
                $stmt->execute([$orderIdToUse,$userId,count($orderItems),$totalQuantity,$totalWeight,$totalCBM,$orderNotes]);
            }
            $orderSummary=[];
            foreach ($orderItems as $item) {
                ...
                $stmt->execute([$orderIdToUse,$item['id'],$item['icode'],(int)$item['quantity'],$unitWeight,$totalItemWeight,$unitCBM,$totalItemCBM]);
            ...
            if ($existingOrderId && $existingOrder) {
                $stmt=$pdo->prepare("UPDATE order_summaries SET total_items=?,total_quantity=?,total_weight=?,total_cbm=?,summary_details=?,updated_at=NOW() WHERE order_id=?");
                $stmt->execute([count($orderItems),$totalQuantity,$totalWeight,$totalCBM,$summaryDetails,$orderIdToUse]);
            } else {
                ...
                $stmt->execute([$orderIdToUse,$userId,count($orderItems),$totalQuantity,$totalWeight,$totalCBM,$summaryDetails]);
            }
            $pdo->commit();
            $_SESSION['order_success']=true; $_SESSION['order_id']=$orderIdToUse;
            ...
            header('Location: order_review.php?order_id='.$orderIdToUse); exit;
        } catch (Exception $e) {
...
.existing-order-banner{display:flex;align-items:center;gap:10px;padding:8px 1rem;background:linear-gradient(90deg,rgba(59,130,246,0.10) 0%,rgba(59,130,246,0.04) 100%);border-bottom:2px solid rgba(59,130,246,0.35);font-size:12px;font-weight:600;color:#1e40af;flex-wrap:wrap}
.existing-order-banner i{color:#3b82f6;font-size:11px}
.existing-order-dot{width:8px;height:8px;border-radius:50%;background:#3b82f6;animation:pulse 1.2s infinite;flex-shrink:0}
...
.msg.info{background:rgba(59,130,246,0.07);color:#1e40af;border-color:#3b82f6}
...
@media(max-width:960px){
    .left-sidebar{position:fixed;top:0;left:0;height:100%;z-index:460;transform:translateX(-100%);width:min(var(--sidebar-w),85vw);box-shadow:var(--shadow-lg);padding-top:var(--hdr-h)}
...
<?php if ($loadingExistingOrder): ?>
<!-- EXISTING ORDER BANNER -->
<div class="existing-order-banner">
    <span class="existing-order-dot"></span>
    <i class="fas fa-clipboard-list"></i>
    Editing <strong>Order #<?php echo htmlspecialchars($existingOrderId); ?></strong> — modify quantities and resubmit to update.
</div>
<?php endif; ?>
    ...
    <div class="hero-banner">
        <div class="hero-center">
            ...
            <div class="hero-banner-title">
                <?php if ($loadingExistingOrder): ?>
                Edit <span>Order #<?php echo htmlspecialchars($existingOrderId); ?></span>
                <?php else: ?>
                ...
                <div class="mobile-cards" id="mobileCards">
                    ...
                    <div class="m-card" data-id="<?php echo htmlspecialchars($item['id']); ?>">
                        ...
                        <div class="m-card-body">
                            <div class="m-card-row"><span class="m-card-lbl">Description</span><span class="m-card-val"><?php echo htmlspecialchars($item['t_size']??'N/A'); ?></span></div>
                            <div class="m-card-row"><span class="m-card-lbl">Weight</span><span class="m-card-val"><?php echo htmlspecialchars($item['fweight']??'—'); ?> kg</span></div>
                            <div class="m-card-row"><span class="m-card-lbl">CBM</span><span class="m-card-val"><?php echo htmlspecialchars($item['cbm']??'—'); ?></span></div>
                        </div>
...
<div class="order-dock" id="orderDock">
    <div class="dock-header" onclick="toggleDock()">
        <div class="dock-title">
            ...
            <?php echo $loadingExistingOrder ? 'Edit Order #'.htmlspecialchars($existingOrderId) : 'Order'; ?>
        ...
        <div class="dock-actions">
            <form id="orderForm" method="POST">
                ...
                <button type="submit" class="dbtn dbtn-submit" onclick="return submitOrder(event)">
                    ...
                    <?php echo $loadingExistingOrder ? 'Update Order' : 'Place Order'; ?>
                </button>
...
const existingOrderId = <?php echo json_encode($existingOrderId); ?>;
const existingOrderItemsData = <?php echo json_encode($existingOrderItems); ?>;
...
function renderTable(items) {
    ...
    items.forEach(item=>{
        const br = item.tire_brand||item.brand||'Unknown';
    ...
    brandGroups.forEach((groupItems, brandName) => {
        tableRows += `<tr class="brand-group-header"><td colspan="10"><div class="brand-group-label"><i class="fas fa-tag"></i>${esc(brandName)}<span class="brand-group-count">${groupItems.length} items</span></div></td></tr>`;
        ...
        groupItems.forEach(item=>{
            ...
            const br = item.tire_brand||item.brand||'';
            ...
            const qtyAttr = `data-id="${esc(item.id)}" data-icode="${esc(item.icode||'')}" data-size="${esc(item.t_size||'')}" data-tiresize="${esc(item.tire_size||'')}" data-brand="${esc(br)}" data-color="${esc(item.col||'')}" data-rim="${esc(item.rim||'')}" data-fweight="${esc(item.fweight||0)}" data-cbm="${esc(item.cbm||0)}" data-customer-price="${esc(cp)}" data-needs-approx-warning="${needsApprox?'1':'0'}"`;
            tableRows += `<tr data-id="${esc(item.id)}" class="${saved[item.id]?'selected':''}">
                <td class="code-cell">${esc(item.icode||'N/A')}</td>
                <td class="desc-cell">${esc(item.t_size||'N/A')}</td>
                ...
                <td><input type="number" class="qty-inp ${hv}" min="0" value="${q}" placeholder="0" ${qtyAttr} onchange="updateOrder(this)" oninput="updateOrder(this)"></td>
            </tr>`;
            mobileCards += `<div class="m-card${saved[item.id]?' selected':''}" data-id="${esc(item.id)}">
                <div class="m-card-header">
                    <div><div class="m-card-code">${esc(item.icode||'N/A')}</div><div class="m-card-brand">${esc(br||'N/A')}</div></div>
                    <div style="text-align:right;">
                        ...
                        <div style="display:flex;gap:3px;justify-content:flex-end;flex-wrap:wrap;margin-top:2px;">
                            ${item.col&&item.col!=='—'?`<span class="color-badge" style="font-size:9px;padding:1px 5px;">${esc(item.col)}</span>`:''}
                            ${item.rim&&item.rim!=='—'?`<span class="rim-badge" style="font-size:9px;padding:1px 5px;">${esc(item.rim)}</span>`:''}
                        </div>
                    </div>
                </div>
                <div class="m-card-body">
                    <div class="m-card-row"><span class="m-card-lbl">Description</span><span class="m-card-val">${esc(item.t_size||'N/A')}</span></div>
                ...
                <div class="m-card-footer">
                    <div class="m-card-price"><div class="m-card-price-lbl">Unit Price</div>${priceHtml}</div>
                    <div class="m-qty-wrap">
                        <button type="button" class="m-qty-btn" onclick="mQtyAdj(this,-1)"><i class="fas fa-minus" style="font-size:10px;"></i></button>
                        <input type="number" class="m-qty-inp qty-inp ${hv}" min="0" value="${q}" placeholder="0" ${qtyAttr} onchange="updateOrder(this)" oninput="updateOrder(this)">
                        <button type="button" class="m-qty-btn" onclick="mQtyAdj(this,1)"><i class="fas fa-plus" style="font-size:10px;"></i></button>
                    </div>
                </div>
            </div>`;
...
function _doUpdateOrder(input, qty) {
    ...
    input.closest('.m-card')?.classList.add('selected');
...
function updateQty(input, id) {
    ...
    if (item) { item.quantity=q; orderItems.set(id,item); syncCompanionInput(id,q); refreshSummary(); saveState(); }
}
...
/* ── State persistence + existing order loading ──────────────────────── */
function saveState() { sessionStorage.setItem('orderItems', JSON.stringify(Array.from(orderItems.values()))); }

function loadExistingOrSavedOrder() {
    // If we have an existing order from PHP, load that first
    if (existingOrderItemsData && existingOrderItemsData.length > 0) {
        let loadedCount = 0;
        existingOrderItemsData.forEach(item => {
            const productId = String(item.product_id);
            const quantity = parseInt(item.quantity) || 0;
            if (quantity <= 0) return;
            const brand = item.tire_brand || item.realstock_brand || '';
            orderItems.set(productId, {
                ...
                color: item.col || '',
                rim: item.rim || '',
                fweight: parseFloat(item.fweight) || parseFloat(item.unit_weight) || 0,
                cbm: parseFloat(item.cbm) || parseFloat(item.unit_cbm) || 0,
                quantity: quantity,
                customerPrice: '',
                approxAcknowledged: true,
            });
            const input = document.querySelector(`.qty-inp[data-id="${productId}"]`);
            if (input) {
        ...
        refreshSummary();
        saveState();
        // Auto-open dock if items loaded
        if (orderItems.size > 0 && !dockOpen) {
            setTimeout(() => toggleDock(), 500);
        }
        return;
    }
