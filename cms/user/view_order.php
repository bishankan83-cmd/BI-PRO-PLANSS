...
$userId    = $_SESSION['id'];
$queryUser = mysqli_query($con, "SELECT fullName, userEmail FROM users WHERE id = '" . mysqli_real_escape_string($con, $userId) . "'");
...
/* ── Special filter: ?filter=to_production (price_pending OR cus_confirmed) ── */
$filterParam  = isset($_GET['filter']) ? trim($_GET['filter']) : '';
$filterStatus = isset($_GET['status']) ? trim($_GET['status']) : '';

$isToProduction = ($filterParam === 'to_production');

/* ── Status map for ?status= param ── */
$statusMap = [
    'pending'        => 'pending',
    ...
    'completed'      => 'completed',
    'complete'       => 'complete',
    'pi_confirm'     => 'pi_confirm',
    'price_pending'  => 'price_pending',
    'cus_confirmed'  => 'cus_confirmed',
    'confirmed'      => 'confirmed',
    'Share_planning' => 'Share_planning',
    'in progress'    => 'in progress',
];
$actualStatus = isset($statusMap[$filterStatus]) ? $statusMap[$filterStatus] : $filterStatus;
...
$baseSql = "
    ...
    WHERE tor.customer_id = ? AND tor.status != 'revised'
";

if ($isToProduction) {
    $baseSql .= " AND (tor.status IN ('price_pending','price pending','cus_confirmed','cus_confirm','customer_confirmed','manager_confirm_disc_success'))";
    $baseSql .= " GROUP BY tor.order_id ORDER BY tor.created_at DESC";
    $stmt = $con->prepare($baseSql);
    if ($stmt) $stmt->bind_param("i", $userId);
} elseif ($actualStatus !== '') {
    $baseSql .= " AND tor.status = ?";
    $baseSql .= " GROUP BY tor.order_id ORDER BY tor.created_at DESC";
    $stmt = $con->prepare($baseSql);
    if ($stmt) $stmt->bind_param("is", $userId, $actualStatus);
} else {
    $baseSql .= " GROUP BY tor.order_id ORDER BY tor.created_at DESC";
    $stmt = $con->prepare($baseSql);
    if ($stmt) $stmt->bind_param("i", $userId);
...
/* ── Reuse eligibility helper ── */
function canReuse($status) {
    $s = strtolower(trim($status));
    return in_array($s, ['complete', 'share_planning']);
}

/* ── Revise eligibility helper — visible for all statuses EXCEPT pending ── */
...
$activeFilterLabel = '';
if ($isToProduction) {
    $activeFilterLabel = 'To Be Production';
} elseif ($filterStatus) {
    $activeFilterLabel = ucwords(str_replace('_', ' ', $filterStatus));
}
...
if ($isToProduction) {
    $pricePendingCnt = count(array_filter($rows_data, function($r) {
        return in_array(strtolower(trim($r['status'])), ['price_pending', 'price pending']);
    }));
    $cusConfirmedCnt = count(array_filter($rows_data, function($r) {
        return in_array(strtolower(trim($r['status'])), ['cus_confirmed', 'cus_confirm', 'customer_confirmed']);
    }));
} else {
...
:root {
    ...
    --prod:      #7c3aed;
    --prod-lt:   rgba(124,58,237,0.10);
    --prod-gl:   rgba(124,58,237,0.22);
    --bg:        #f0f2f5;
    --white:     #ffffff;
    --g50:       #f9fafb;
    --g100:      #f3f4f6;
...
.prod-banner-text { flex:1; }
.prod-banner-title { font-size:1rem; font-weight:800; margin-bottom:2px; }
.prod-banner-sub   { font-size:0.82rem; opacity:0.9; }
.prod-banner-close {
    color:rgba(255,255,255,0.7); font-size:1rem; cursor:pointer;
    padding:4px 8px; border-radius:6px; transition:var(--tr);
    background:none; border:none; font-family:var(--font);
}
.prod-banner-close:hover { background:rgba(255,255,255,0.15); color:#fff; }
...
.eyebrow::before { content:''; width:14px; height:2px; background:var(--orange); border-radius:2px; }
.page-title {
...
.fchip.prod-chip { background:var(--prod); }
...
.sico {
    ...
    display:flex; align-items:center; justify-content:center;
    font-size:15px; color:var(--orange); flex-shrink:0;
}
.sico.prod { background:var(--prod-lt); color:var(--prod); }
...
.tb-ico.prod { background:var(--prod); }
...
.cpill.prod { background:var(--prod-lt); color:var(--prod); border-color:rgba(124,58,237,0.2); }
...
table.t col.c6  { width:158px; }
table.t col.c7  { width:230px; }
...
table.t th i.hi { color:var(--orange); margin-right:5px; font-size:9px; }
...
table.t tbody tr:hover { background:rgba(242,128,24,0.03); }

table.t tbody tr.prod-row { background:rgba(124,58,237,0.03); }
table.t tbody tr.prod-row:hover { background:rgba(124,58,237,0.07); }
...
.rbtn:hover {
    ...
    box-shadow:0 2px 8px rgba(91,192,190,0.35);
}
.rbtn i { font-size:10px; }
...
<header class="hdr">
    <div class="hdr-inner">
        ...
        <div class="hdr-r">
            <a href="dashboard.php" class="hbtn"><i class="fas fa-arrow-left"></i> Dashboard</a>
            <a href="add_order.php" class="hbtn pri"><i class="fas fa-plus"></i> New Order</a>
...
<div class="wrap">
    ...
    <?php if ($isToProduction): ?>
    <!-- To-Be-Production Banner -->
    <div class="prod-banner" id="prodBanner">
        <div class="prod-banner-ico">
            <i class="fas fa-shipping-fast"></i>
        </div>
        <div class="prod-banner-text">
            <div class="prod-banner-title">To Be Production Orders</div>
            <div class="prod-banner-sub">
                Showing <?php echo $totalOrders; ?> order<?php echo $totalOrders !== 1 ? 's' : ''; ?> ready for production —
                <?php echo $pricePendingCnt; ?> Price Pending &amp; <?php echo $cusConfirmedCnt; ?> Customer Confirmed
            </div>
        </div>
        <button class="prod-banner-close" onclick="document.getElementById('prodBanner').style.display='none'">
    ...
    <div class="hero">
        <div>
            ...
            <div class="page-title">
                ...
                <?php if ($isToProduction): ?>
                    <span class="fchip prod-chip"><i class="fas fa-shipping-fast" style="font-size:9px;"></i> To Be Production</span>
                <?php elseif ($filterStatus): ?>
                    <span class="fchip"><i class="fas fa-filter" style="font-size:9px;"></i> <?php echo htmlspecialchars($activeFilterLabel); ?></span>
                <?php endif; ?>
            </div>
            <div class="sub">
                <?php
                if ($isToProduction) {
                    echo 'Showing orders with status: <strong>Price Pending</strong> or <strong>Customer Confirmed</strong>';
                } elseif ($filterStatus) {
                    echo 'Showing orders with status: <strong>' . htmlspecialchars($activeFilterLabel) . '</strong>';
                } else {
                    echo 'View and manage all your submitted orders.';
                }
                ?>
            </div>
        </div>
        <?php if ($isToProduction || $filterStatus): ?>
            <a href="view_order.php" class="hbtn"><i class="fas fa-times"></i> Clear Filter</a>
        <?php endif; ?>
    </div>
    ...
    <div class="stats">
        <div class="scard">
            <div class="sico <?php echo $isToProduction ? 'prod' : ''; ?>">
                <i class="fas <?php echo $isToProduction ? 'fa-shipping-fast' : 'fa-receipt'; ?>"></i>
            </div>
            <div>
                <div class="slabel"><?php echo $isToProduction ? 'Total To-Be-Prod.' : 'Total Orders'; ?></div>
                <div class="sval"><?php echo $totalOrders; ?></div>
            </div>
        </div>
        <div class="scard">
            <div class="sico"><i class="fas fa-layer-group"></i></div>
            <div>
        ...
        <?php if ($isToProduction): ?>
        <div class="scard">
            <div class="sico prod"><i class="fas fa-hourglass-half"></i></div>
            <div>
                <div class="slabel">Price Pending</div>
                <div class="sval"><?php echo $pricePendingCnt; ?></div>
            </div>
        </div>
        <?php else: ?>
        <div class="scard">
            <div class="sico"><i class="fas fa-clock"></i></div>
            <div>
    ...
    <div class="panel">
        ...
        <div class="toolbar">
            <div class="tb-l">
                <div class="tb-ico <?php echo $isToProduction ? 'prod' : ''; ?>">
                    <i class="fas <?php echo $isToProduction ? 'fa-shipping-fast' : 'fa-history'; ?>"></i>
                </div>
                <div class="tb-title"><?php echo $isToProduction ? 'To Be Production' : 'Your Orders'; ?></div>
                <span class="cpill <?php echo $isToProduction ? 'prod' : ''; ?>" id="cPill">
            ...
            <div class="tb-r">
                ...
                <?php if ($isToProduction || $filterStatus): ?>
                    <a href="view_order.php" class="hbtn"><i class="fas fa-times"></i> Clear</a>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="tbl-wrap">
            <table class="t">
                ...
                <?php
                    ...
                    [$bCls, $bIco, $bLbl] = getBadge($row['status'] ?? 'pending');
                    ...
                    $showReuse  = canReuse($row['status'] ?? '');
                    $showRevise = canRevise($row['status'] ?? '');
                    $rowClass   = $isToProduction ? 'prod-row' : '';
                ?>
                <tr class="<?php echo $rowClass; ?>"
                    ...
                    <td class="c7">
                        <div class="action-group">
                            ...
                            <?php if ($showReuse): ?>
                            <a href="edit_order_enter.php?oid=<?php echo urlencode($row['order_id']); ?>" class="rbtn">
                                <i class="fas fa-redo"></i> Reuse
                            </a>
                            <?php endif; ?>
                            <?php if ($showRevise): ?>
                            <a href="revise_orders.php?oid=<?php echo urlencode($row['order_id']); ?>" class="wbtn">
                ...
                <tr><td colspan="9">
                    <div class="empty">
                        <div class="empty-ico">
                            <i class="fas <?php echo $isToProduction ? 'fa-shipping-fast' : 'fa-inbox'; ?>"></i>
                        </div>
                        <h3>
                            <?php
                            if ($isToProduction)       echo 'No To Be Production Orders';
                            elseif ($filterStatus)     echo 'No ' . htmlspecialchars($activeFilterLabel) . ' Orders';
                            else                       echo 'No Orders Yet';
                            ?>
                        </h3>
                        <p>
                            <?php
                            if ($isToProduction)       echo "You don't have any orders with Price Pending or Customer Confirmed status.";
                            elseif ($filterStatus)     echo "You don't have any orders with this status.";
                            else                       echo "You haven't placed any orders yet.";
                            ?>
                        </p>
                        <div class="empty-acts">
                            <?php if ($isToProduction || $filterStatus): ?>
                                <a href="view_order.php" class="hbtn"><i class="fas fa-list"></i> All Orders</a>
                            <?php endif; ?>
                            <a href="add_order.php" class="vbtn"><i class="fas fa-plus"></i> Place an Order</a>
                        </div>
                    </div>
                </td></tr>
                <?php endif; ?>
