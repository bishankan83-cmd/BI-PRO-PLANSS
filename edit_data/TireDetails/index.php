...
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $result = $conn->query("SELECT * FROM tire_details WHERE icode LIKE '%$search%'");
} else {
    $result = $conn->query("SELECT * FROM tire_details");
}
?>
...
<body>
    <h1 style="text-align: center; color:rgb(11, 11, 11); font-size: 60px;">Tire Details</h1>
    ...
    <div class="table-wrapper">
        <table>
            ...
            <tr>
                ...
                <td><?= htmlspecialchars($row['greenweight']) ?></td>
                <td><?= htmlspecialchars($row['stgreenweight']) ?></td>
                <td><?= htmlspecialchars($row['fweight']) ?></td>
                <td><?= htmlspecialchars($row['cbm']) ?></td>
                <td><?= htmlspecialchars($row['maxload']) ?></td>
                <td>
                    <a href="edit.php?icode=<?= urlencode($row['icode']) ?>" style="padding: 5px; background-color: #28a745; color: white; border: none; border-radius: 5px; text-decoration: none;">Edit</a> |
                    <a href="delete.php?icode=<?= urlencode($row['icode']) ?>" style="padding: 5px; background-color: #dc3545; color: white; border: none; border-radius: 5px; text-decoration: none;" onclick="return confirm('Are you sure?')">Delete</a>
                </td>
            </tr>
            <?php endwhile; ?>
        </table>
