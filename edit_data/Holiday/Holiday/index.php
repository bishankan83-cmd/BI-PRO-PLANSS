...
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $result = $conn->query("SELECT DATE(holiday_date) AS holiday_date FROM holidays WHERE holiday_date LIKE '%$search%'");
} else {
    $result = $conn->query("SELECT DATE(holiday_date) AS holiday_date FROM holidays");
}
?>
        ...
        <?php while ($row = $result->fetch_assoc()): ?>
            <tr>
                ...
                <td style="text-align: right;">
                    <a href="edit.php?holiday_date=<?= htmlspecialchars($row['holiday_date']) ?>" class="btn-edit">Edit</a> |
                    <a href="delete.php?holiday_date=<?= htmlspecialchars($row['holiday_date']) ?>" class="btn-delete" onclick="return confirm('Are you sure?')">Delete</a>
                </td>
            </tr>
        <?php endwhile; ?>
    </table>
