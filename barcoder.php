...
function generateQRCode($pdf, $row, $x, $y, $qrCodeSize) {
    // Important data
    $importantData = //'SG-' . $row['sg_value'] . 
                    // '|CN-' . $row['compound_name'] . 
                    // '|QAD-' . $row['quality_approved'] . 
                     '|ED-' . $row['expire_date'] . 
                     '|BN-' . $row['batch'] . 
                    // '|P-' . $row['pallet'] . 
                     '|W-' . $row['weight'];
    ...
    $pdf->MultiCell(0, 0, "Batch:" . $row['batch'], 0, 'L');
...
$sql = "SELECT * FROM another_table_name11"; // Adjust the table name as per your database structure
