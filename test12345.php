...
<head>
    ...
    <title>Process Table Data Check</title>
    <!-- Bootstrap CSS -->
    ...
    <div class="container">
        ...
        <div class="result">
            ...
            if ($conn->connect_error) {
                echo "<div class='alert alert-danger' role='alert'>Connection failed: " . htmlspecialchars($conn->connect_error) . "</div>";
            } else {
                // Query to check if there's any data in the process and process_plan tables
                $sql_process = "SELECT COUNT(*) AS total FROM process";
                $sql_process_plan = "SELECT COUNT(*) AS total FROM process_plan";

                // Execute queries
                $result_process = $conn->query($sql_process);
                $result_process_plan = $conn->query($sql_process_plan);

                // Check process table results
                if ($result_process) {
                    $row_process = $result_process->fetch_assoc();
                } else {
                    echo "<div class='alert alert-danger' role='alert'>Error in process query: " . htmlspecialchars($conn->error) . "</div>";
                }

                // Check process_plan table results
                if ($result_process_plan) {
                    $row_process_plan = $result_process_plan->fetch_assoc();
                } else {
                    echo "<div class='alert alert-danger' role='alert'>Error in process_plan query: " . htmlspecialchars($conn->error) . "</div>";
                }

                // Determine if data exists in both tables
                if ($row_process['total'] > 0 || $row_process_plan['total'] > 0) {
                    echo "<div class='alert alert-warning blink' role='alert'>
                            <strong>Please Note:</strong> The planning department is currently updating the Planning. 
                            Data presented may not be accurate. Please try again later.
                          </div>";
                    echo "<script>
                            setTimeout(function() {
                                alert('Please be advised that the planning department is undertaking updates. As a result, the data presented may not be accurate. We kindly ask you to try again later.');
                            }, 500);
                          </script>";
                } else {
                    echo "<div class='alert alert-success' role='alert'>
                            <strong>Status:</strong> All Work Orders up to date. Data is accurate and ready for use.
                          </div>";
                }
    ...
    foreach ($groupedData as $date => $entries) {
      ...
      //  echo "<td>$sumPlanDifference</td>";
       ...
       // echo "<td>$numCavityIDs</td>";
        ...
        $totalCavityNos = 137; // Assuming Total Cavity is always 130
...
if ($result->num_rows > 0) {
    ...
    foreach ($groupedData as $date => $entries) {
        ...
        // Calculate Average Utilization Percentage and round it to Sankyat
        $utilizedCavityNos = $numCavityIDs;
        $totalCavityNos = 137; // Assuming Total Cavity is always 137
        $averageUtilization = round(($utilizedCavityNos / $totalCavityNos) * 100, 0); // Round to Sankyat
    }
...
$sql = "INSERT INTO calculated_data23 (erp, icode, mold_id, cavity_id, shift, date, plan) 
        SELECT erp, icode, mold_id, cavity_id, shift, date, plan FROM plan_by_date_shift";
...
// SQL query to insert data from calculated_data_stock to calculated_data
$sql = "
    INSERT INTO `calculated_data` (
        ...
        `end_date`, 
        `plan`, 
        `calculated_green_tire_weight`, 
        `calculated_stgreen_tire_weight`
    )
    SELECT 
        `date`, 
        `erp`, 
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Cavity Utilization Summary</title>
    <style>
        body {
            font-family: 'Arial', sans-serif;
            background-color: #f8f9fa;
        ...
        .work-order-td {
            background-color: #feb264; /* Light blue background for Work Order Tires Nos */
        ...
        .work-order-utilized-td {
            background-color: #FEB264; /* Light orange background for Work Order Utilized/Plan Cavity Nos */
...
<div class="container">
    <h1>Cavity Utilization Summary</h1>
    ...
    $sql = "SELECT date,
                   ...
                   SUM(CASE WHEN erp = 1 THEN plan ELSE 0 END) AS total_stock_order_tires,
                   COUNT(DISTINCT CASE WHEN erp = 1 THEN cavity_id END) AS stock_utilized_per_plan_cavity
    ...
    $result = $conn->query($sql);
     ...
     if ($result->num_rows > 0) {
        ...
        echo "<tr>
                <th>Date</th>
                <th>Plan Tires Nos</th>
                <th>Stock Order Tires Nos</th>
                <th class='work-order-header'>Work Order Tires Nos</th>
                <th>Utilized/Plan Cavity Nos</th>
                <th>Stock Utilized/Plan Cavity Nos</th>
                <th class='work-order-utilized-header'>Work Order Utilized/Plan Cavity Nos</th> <!-- New Column -->
                <th>Total Cavity</th>
                <th>Average Utilization (%)</th>
                <th>Total Green Tire Weight</th>
              </tr>";

        while ($row = $result->fetch_assoc()) {
            $totalPlanTires = $row["total_data_plan_amount"];
            $totalStockOrderTires = $row["total_stock_order_tires"];
            $workOrderTiresNos = $totalPlanTires - $totalStockOrderTires;
            $totalCavity = 137; // Fixed value, consider dynamic if needed

            $utilizationPercentage = ($row["unique_cavity_id_quantity"] / $totalCavity) * 100;

            // Calculate Work Order Utilized/Plan Cavity Nos
            $workOrderUtilizedPerPlanCavity = $row["unique_cavity_id_quantity"] - $row["stock_utilized_per_plan_cavity"];

            echo "<tr>
                    <td><strong>" . $row["date"] . "</strong></td>
                    <td>{$totalPlanTires}</td>
                    <td>{$totalStockOrderTires}</td>
                    <td class='work-order-td'>{$workOrderTiresNos}</td> <!-- Light blue background -->
                    <td>{$row["unique_cavity_id_quantity"]}</td>
                    <td>" . number_format($row["stock_utilized_per_plan_cavity"]) . "</td>
                    <td class='work-order-utilized-td'>" . number_format($workOrderUtilizedPerPlanCavity) . "</td> <!-- Light orange background -->
                    <td>{$totalCavity}</td>
                    <td>" . number_format($utilizationPercentage, 2) . "%</td>
                    <td>{$row["total_green_tire_weight"]}</td>
                  </tr>";
        }
    ...
    <div class="footer">
        <p>&copy; 2024 ATIRE (PVT) LTD. All rights reserved.</p>
    </div>
</div>
