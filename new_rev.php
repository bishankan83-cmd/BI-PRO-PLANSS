...
try {
    ...
    // SQL query to delete data from the rworder table
...
try {
    // Delete records from `worder` table
    $deleteWordOrder = "
        DELETE FROM `worder`
        WHERE `erp` IN (
            SELECT `erp` FROM `worder_result`
        )
    ";
    $conn->query($deleteWordOrder);

    // Delete records from `work_order` table
    $deleteWorkOrder = "
