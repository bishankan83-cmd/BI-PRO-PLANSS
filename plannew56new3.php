...
$sql = "SELECT s.id, s.icode, s.tires_per_mold, s.mold_id, s.cavity_id, t.time_taken, m.availability_date AS mold_availability, c.availability_date AS cavity_availability, s.start_date AS process_start_date, s.erp, op.start_date AS old_process_start_date
        FROM process s
        INNER JOIN tire t ON s.icode = t.icode
        LEFT JOIN mold m ON s.mold_id = m.mold_id
        LEFT JOIN cavity c ON s.cavity_id = c.cavity_id
        LEFT JOIN old_process op ON s.erp = op.erp AND s.mold_id = op.mold_id AND s.cavity_id = op.cavity_id AND s.icode = op.icode
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $start_date = max(
            ...
            $row['old_process_start_date'] ?: date("Y-m-d H:i:s")
        );
