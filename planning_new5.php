...
<body>
    ...
    if (isset($_GET['mold_id']) && !empty($_GET['mold_id'])) {
        $mold_id = $_GET['mold_id'];
        // Modify the SQL query to include the mold_id condition
        $erpSql = "SELECT erp, customer, MAX(end_date) as last_completion_date FROM plannew1 WHERE mold_id = '$mold_id' GROUP BY erp";
    } else {
    ...
    if ($erpResult) {
        if (mysqli_num_rows($erpResult) > 0) {
            ...
            while ($erpRow = mysqli_fetch_assoc($erpResult)) {
                ...
                if (isset($_GET['mold_id']) && !empty($_GET['mold_id'])) {
                    $sql = "SELECT * FROM plannew1 WHERE erp = '$erp' AND mold_id = '$mold_id'";
                } else {
