...
$sql = "
    ...
    FROM BatchRange br
    JOIN `bcompound98` b ON br.`id` = b.`id` AND br.`batch` BETWEEN b.`batch` AND b.`batch2`
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $insertSql = "INSERT INTO bcompoundr (id, inputDate, shift, compound_name, description, cstock, batch, pallet, created_at, weight, serial_number) VALUES (";
        $insertSql .= "'" . $row["id"] . "', ";
...
$sql = "DELETE FROM another_table_name33";
...
// SQL query to delete all data from the table
$sql = "DELETE FROM another_table_name11";
...
$sql = "UPDATE `bcompoundr` SET `batch` = CONCAT('0', `batch`) WHERE `batch` REGEXP '^[1-9]$'";
...
$sql_select = "SELECT * FROM bcompoundr WHERE 1";
...
$params = array();
    ...
    if ($result->num_rows > 0) {
        ...
        $sql_insert = "INSERT INTO another_table_name33 (inputDate, shift, compound_name, description, cstock, batch, pallet, created_at, weight, serial_number) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
<script>
    ...
    function populateSerialNumberOptions(selectedDate) {
        ...
        xhr.open("POST", "get_batch1r.php", true);
    ...
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "get_compound_namesr.php", true);
    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
...
function populateBatchOptions(selectedDate) {
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "get_batch_re.php", true);
    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
