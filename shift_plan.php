...
$createTableQuery = "
    CREATE TABLE IF NOT EXISTS calculated_data23 (
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        foreach ($dateRange as $date) {
            ...
            $deleteQuery = "DELETE FROM calculated_data23 WHERE date = '$currentDate'";
            ...
            $groupedData[$currentDate][] = [
                ...
                'User Time' => $endDateMidnight->format('Y-m-d 07:00:00'), // Separate field with time set to 07:00:00
    ...
    foreach ($groupedData as $date => $entries) {
        ...
        // Add a new column for Total Cavity and set each value to 134
        $totalCavityNos = 134; // Assuming Total Cavity is always 134

        // Calculate Average Utilization Percentage and round it to 2 decimal places
        $utilizedCavityNos = $numCavityIDs;
        $averageUtilization = round(($utilizedCavityNos / $totalCavityNos) * 100, 0); // Round to 0 decimal places
