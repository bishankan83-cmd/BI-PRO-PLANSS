...
function fetchData($conn) {
    ...
    LEFT JOIN tobeplan ON process_plan.icode = tobeplan.icode
    ...
    WHERE process_plan.is_highlighted = 1
    GROUP BY process_plan.id, process_plan.icode, process_plan.mold_name, process_plan.tires_per_mold, process_plan.cavity_name, process_plan.is_completed, process_plan.is_highlighted, tire_details.description, tobeplan.tobe";  
    $result = $conn->query($sql);
...
<input type="text" id="searchChuringGroup" oninput="filterTable('churingGroup')" placeholder="Type to search...">
  ...
  <br>
  <button type="button" onclick="redirectToAnotherPages()">GO MAIN</button>
</td>
